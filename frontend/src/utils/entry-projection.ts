 ...
 * AskUserQuestion 答复为 typed 条目、子任务条目带 parent_tool_use_id、
 ...
 *   完成、子任务子时间线挂载都在这层重算；按 turn 版本缓存，只有被新
...
/** tool_use 块的登记位置：跨 turn 回填与子任务锚定都按此 O(1) 定位。 */
...
/** 一条平铺时间线（主时间线或某个子任务组）的折叠状态。 */
interface Fold {
  committed: InternalTurn[];
  cursor: InternalTurn | null;
  /** 所属子任务组（主时间线为 null），脏标记沿锚链向上传播用。 */
...
export function createTimelineProjector(): TimelineProjector {
  ...
  // 里已锚定完成的子任务组随会话增长把每条新消息的合成卡片扫描拖成
  // O(历史子任务总数)。
   ...
   * 用于锚定前判定：真实数据里子任务嵌套是无环 DAG（parent_tool_use_id
  ...
  /** 条目触达 turn 后：失效其展示缓存，并沿子任务锚链逐层失效祖先 turn。 */
  ...
  /** 登记 tool_use 块位置（同 id 首次登记生效），并锚定等待中的子任务组。 */
  ...
  /**
   * 构建单个 turn 的展示视图：挂载子任务子时间线、把已完成 `Agent` 调用
  ...
  function compose(): Turn[] {
    ...
    // groups——已锚定的组不会再变回待锚定，扫描量不随会话内子任务总数增长。
