...
export function ProviderSection() {
  ...
  const { ownProviders, presetProviders } = useMemo(() => {
    const own = providers.filter((p) => p.group === "own");
    const preset = providers.filter((p) => p.group !== "own");
    return { ownProviders: own, presetProviders: preset };
  }, [providers]);
  ...
  const renderProvider = (p: ProviderInfo) => {
    const isActive = selection?.kind === "preset" && selection.id === p.id;
    return (
      <button
        key={p.id}
        type="button"
      ...
      >
        {ownProviders.length > 0 && (
          <>
            ...
            {ownProviders.map(renderProvider)}
          </>
        )}
        <div className="mb-2 px-3 font-mono text-[9.5px] font-bold uppercase tracking-[0.16em] text-text-4">
          {t("preset_providers")}
        </div>
        {presetProviders.map(renderProvider)}
