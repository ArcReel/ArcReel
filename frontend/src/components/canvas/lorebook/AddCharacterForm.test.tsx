...
describe("AddCharacterForm", () => {
  ...
  it("submits an optional reference file together with the new character", async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText("Character appearance, personality, background, etc..."),
      {
        target: { value: "hero desc" },
      },
    );
    fireEvent.change(screen.getByPlaceholderText("Optional, e.g.: gentle yet authoritative"), {
      target: { value: "warm" },
    });
