...
function mkPreview(overrides: Partial<ScriptPreview> = {}): ScriptPreview {
  return {
    utterances: [{ index: 1, kind: "dialogue", speaker: "张三", text: "我来了" }],
    warnings: [],
    ...overrides,
  };
}
...
describe("ScriptPreviewPanel", () => {
  it("renders utterance counts and warnings from the server", async () => {
  ...
  it("shows an unresolved-mention warning without blocking anything", async () => {
    vi.spyOn(API, "previewReferenceScript").mockResolvedValue(
      mkPreview({
        warnings: [{ key: "ref_warn_unregistered_mention", message: "「路人」未登记，不会成为参考图" }],
      }),
    );
  ...
  it("debounces edits into a single request", async () => {
    ...
    const { rerender } = renderPanel("中");
    rerender(<ScriptPreviewPanel projectName="demo" episode={1} text="中景" lookup={LOOKUP} />);
    rerender(<ScriptPreviewPanel projectName="demo" episode={1} text="中景。" lookup={LOOKUP} />);
    await vi.advanceTimersByTimeAsync(500);

    await waitFor(() => expect(spy).toHaveBeenCalledTimes(1));
    expect(spy).toHaveBeenCalledWith("demo", 1, "中景。", expect.anything());
  });
  ...
  it("aborts an in-flight request as soon as the body changes", async () => {
  ...
  it("refetches when the project assets behind the warnings change", async () => {
    const spy = vi.spyOn(API, "previewReferenceScript").mockResolvedValue(mkPreview());
    const { rerender } = renderPanel("@酒馆 内景。");
    await vi.advanceTimersByTimeAsync(500);
    await waitFor(() => expect(spy).toHaveBeenCalledTimes(1));
