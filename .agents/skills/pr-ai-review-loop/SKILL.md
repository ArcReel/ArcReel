...
**驱动方式(重要):** skill 自带 self-pace —— 每轮 poll 决策完成后,调用 `ScheduleWakeup(delaySeconds=N, prompt='/pr-ai-review-loop 继续 PR #<N> review loop 第 <round+1> 轮 poll', reason='...')` 排下次唤醒,harness 自动回调本 skill。**不需要**外层 `/loop` 模式包裹,也**不需要**用户手动催 "继续"。`ScheduleWakeup` 工具在任意 session 都可直接调用(spec 描述虽限定 `/loop` dynamic mode,但实测无硬校验)。唤醒间隔见 §「polling 节奏」。
...
| 触发 `/gemini review` / `@codex review` 后 | **120s** | dogfood 实测 Gemini 响应常 90-120s,60s 易扑空 |
