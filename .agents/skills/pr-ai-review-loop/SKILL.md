...
PR push 上去之后,CodeRabbit / Gemini / Codex 这几家 AI reviewer 会反复 review → 你修 → push → 再 review。这套循环由本 skill 接手:盯状态、该催的催、把意见收拢后扔给 `receiving-code-review` 处理。

## 运行模式:无人值守 + 两类停问

本 skill 的设计就是自动跑完整个循环,不要每轮停下来征求授权。该不该触发命令、要不要 push 修复、回不回 inline、什么时候下一轮 poll,按决策表自己拍板。

**驱动方式(重要):** 本 skill 自带 self-pace。每轮 poll + 决策做完后,直接调 `ScheduleWakeup` 排下一次唤醒,到点 harness 会自动重新进入本 skill。**不用**外层套 `/loop`,也**不用**等用户敲"继续"。`delaySeconds` 看下方「polling 节奏」表;`prompt` 字段按当前 PR 号和轮数自己组织,能让 harness 重新拉起本 skill 就行。
...
- **根本性分歧没定论** —— 同一个**主题指纹**(reviewer + 关键词,比如 "Pydantic `extra=ignore` vs `forbid`")同一家 reviewer 连提 ≥ 2 轮,而且没有 ADR / memory 兜底 → 停下来问"要不要开 ADR PR / 你来拍板"
- **reviewer 之间打架** —— 同一件事 A 家说 X、B 家说不该 X → 停下来让用户裁判,不要自己站队
- **业务取舍** —— 修复方案在前向兼容 / 性能 / 用户体验上有明显差别,可能踩到业务意图 → 停下来确认

> 调度停问不等于违背无人值守。无人值守指**循环里能自己拍板的动作**(poll / 触发 / 合并意见 / push)继续自己拍;只有**超出本 skill 调度范围的根本性争议**才丢给用户。

主题指纹由 Claude 在对话上下文里维护 `topic_history`(每轮追加"reviewer + 一句话主题摘要"),靠语义相似度判同主题。不脚本化、不落盘。

其它一切(cold-start 等多久、要不要发 `/gemini review`、ack 还是 actionable、要不要叫 Codex、新 HEAD 后什么时候回来 poll、commit / push 节奏)都自己拍。
...
每条 commit 输出 `{files_changed, lines_added, message_head, ...}`。如果本轮 push 的 commit **全部**是 fix-up(nit / format / typo / 改一个字段 / 小 bug),Claude 自己判一下 → **手动 `/gemini review` / `@codex review` 这一轮就跳过**,等 CodeRabbit 自动跟就行(CR 跟新 commit 是自动的,不烧 quota)。理由:Gemini / Codex 一来就是整个 PR 全审一遍,quota 烧得起的次数有限。
...
| 还有 reviewer 没在最新 HEAD 上出结果 | 等下一轮(看下方「polling 节奏」) |
| 至少一家 reviewer 给了新的 actionable 意见 | 进步骤 3 |
| 所有启用的 reviewer 都对当前 HEAD 亮绿灯(看下方「怎么算已通过」) | 退出,简短汇报 |
...
receiving-code-review 调完回步骤 1。它自己负责动手修、给 reviewer 回 inline、记 pushback —— 本 skill 只回头拉数据,看有没有新 HEAD 或新一轮 review 出来。
...
- 或者这家 reviewer 被用户临时关掉

### Codex 触发决策

Codex 跟不跟新 commit 看仓库配置(详见 references/reviewers.md)。仓库没开自动 review 的话,要不要手动 `@codex review` 自己拍板:

- 用户的明确意图(只要提到 codex 基本就是要叫)
- CodeRabbit 和 Gemini 意见冲突,要不要第三方仲裁
- PR 改动面值不值得多看一遍(敏感模块、跨模块影响、新加依赖这类)
- 本 HEAD 上是不是已经叫过了(去重)
- **保守跳过有没有命中**(本轮全是 fix-up)

### polling 节奏

每轮 poll + 决策做完就调 `ScheduleWakeup` 排下一次唤醒(看上面「驱动方式」)。`delaySeconds` 按下表:
...
2. **连续 2 轮 `last_commit_shapes` 全是 nit/format**(`classify_commits.sh` 输出 + Claude 自己判) → 停下来问"再改下去边际收益小了,要不要收?"
3. **同一个 `topic_history` 主题指纹连提 ≥ 3 轮**(和上面「运行模式 B」联动) → 停下来问要不要升级 ADR
...
- **某家 reviewer 一直不回**:bot 可能挂了或者配额满了。15 分钟没动静就停下来问用户(跟 polling 节奏的上限一致)
- **bot 报错**("Internal error" / "Token limit exceeded"):把错误内容贴给用户,问要不要发 `@coderabbitai full review` / `/gemini review` 强制重跑
- **`poll.quota_alerts` 不为空**:bot 在 PR 里留了 quota / rate limit 报错 —— 把 `body_head` 贴给用户,问要不要先把这家关掉继续别家,或者等 quota 回血再 push
- **`gh` 401/403**:让用户跑 `gh auth refresh -s repo`
- **`poll.sh` / `classify_commits.sh` 报 `POLL_ERROR:`**:重试一次(网络抖动常见),再失败就把 stderr 贴给用户
- **CI 失败**:CodeRabbit 会等 GitHub Checks 跑完再继续;CI 红的时候 review 可能根本不来 —— 先帮用户修 CI

## 跟别的 skill 怎么分

| 任务 | 用哪个 |
|---|---|
| 建 PR | `commit-commands:commit-push-pr` |
| 回 / 实施 / 反驳 review 意见 | `receiving-code-review` |
| 验证修复是不是真的解决问题 | `verify` |
| **盯多家 AI reviewer 的循环节奏** | **本 skill** |

本 skill 只管调度 —— 什么时候 poll、什么时候 resume / 触发、什么时候把活交给 receiving-code-review、什么时候收尾。**不**管"意见怎么回"和"修复怎么验证"。
