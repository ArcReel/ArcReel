...
有证据仍无法消解的 reviewer 冲突、重大业务取舍或重要风险，及时交委派方裁决；普通技术分歧自行判断。退出时报告最终 HEAD、轮数、验证结果、关键修复、不修改的依据和未决事项。只有确有超出现有记录或范围的长期价值时，才作为 follow-up 线索报告，附依据并区分事实与猜测；分类与价值评估留给委派方，通常无需新增。
