...
**驱动方式(重要):** skill 自带 self-pace —— 每轮 poll 决策完成后,用 `ScheduleWakeup` 工具排下次唤醒,harness 在指定延迟后自动回调本 skill。**不需要**外层 `/loop` 模式包裹,也**不需要**用户手动催 "继续"。`ScheduleWakeup` 工具在任意 session 都可直接调用(spec 描述虽限定 `/loop` dynamic mode,但实测无硬校验)。`delaySeconds` 见 §「polling 节奏」表;`prompt` 字段按当前上下文(PR 号、轮数)自行组织,能让 harness 重新启动本 skill 即可。
