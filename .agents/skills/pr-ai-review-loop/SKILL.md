...
| `security_alerts.open_introduced` 非空但无对应新评论,且未全部计入已认定误报在案清单(reviewers.md「已知误报」) | 上一轮没修干净(bot 不重复提醒)——把 alert 数据(rule / path / url)直接带入步骤 3,按数据修而非按评论修。前提:CodeQL 分析完成且成功(门槛 1 口径)——分析未完成时差集基于过期数据,归入下行等待 |
