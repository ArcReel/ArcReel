...
设计依据(节奏控制、状态字段语义、quota 经济学等)见 [references/internals.md](references/internals.md)。本文档专注于执行规则。
...
调度类暂停不违背无人值守原则——无人值守指循环内可自行决定的动作(poll、触发、合并意见、push)继续自行决定;仅超出本 skill 调度范围的根本性争议才升级到用户。

主题指纹由 Claude 在对话上下文中维护 `topic_history`,以语义相似度判定同主题。详细机制见 internals.md。
...
字段语义与设计踩坑见 `scripts/poll.sh` 头部注释,首次进循环时阅读一次,后续仅看 JSON 输出。JSON 解析后仅保留在对话上下文中,不落盘。
...
若本轮 push 的所有 commit 均属 fix-up(nit、format、typo、单字段调整、小 bug 修复),本轮**跳过**手动触发 Gemini 与 Codex,等 CodeRabbit 自动跟即可。CodeRabbit 跟新 commit 不消耗 quota;Gemini 与 Codex 每次手动触发都会重新全审 PR,quota 应节约使用。
