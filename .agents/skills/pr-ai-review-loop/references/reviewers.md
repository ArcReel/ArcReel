...
| OpenAI Codex | `chatgpt-codex-connector` | `chatgpt-codex-connector[bot]` | **按仓库配置**:默认要手动 `@codex review`;某些仓库(如 ArcReel/ArcReel)开了 PR 自动 review,Codex 会自动跟新 commit | **三种 ack 模式**——见下方 | `@codex review` |
...
3. **empty-body review**:`codex.reviews` 里最新一条 `submittedAt > last_push_at` **且** `state == "COMMENTED"` **且** `body == ""`,且本轮无新 inline。这是较新 Codex 行为——在新 HEAD 自动跟时若无新意见,可能用空 body review 代替 reaction
