...
Gemini 可沿用最近一次已完成且发现已处置的审查，但须检查从**最近已审 commit**到当前 HEAD 的全部变更，确认没有改变已审行为或指令含义，例如仅有格式或拼写调整。行为、契约和风险变化需要复审；小 bug 修复同样是语义变化。

`classify_commits.sh` 可帮助定位提交，不能单凭提交名或改动大小决定沿用。需查看提交范围时按其 header 使用；出现 `SINCE_SHA ... is not on PR`，或无法确认覆盖连续性时，重新审查。沿用时在 PR 留下已审 SHA、当前 SHA 和依据，便于接力核对。
