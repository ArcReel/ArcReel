...
**已知误报**:`py/path-injection` 告警中,路径经 `ProjectManager.get_project_path()`(内部 `safe_join` 消毒)获得的属已知误报家族——核实告警链路确实经过该函数后,处置为 PR 评论说明 + 转呈用户 dismiss(reason: false positive),不重新分析、不为其改代码。dismiss 权限在用户,不得代执行。
