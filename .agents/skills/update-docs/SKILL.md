...
按最近的代码改动更新面向用户的文档。两个引擎：缺漏引擎（gap finder）读 git 历史，找已落地但文档未反映的新能力；核对引擎（fact checker）读文档，找与代码现状不符的过时内容。措辞与结构改动直接修改，事实项经用户确认后再改。

## 适用范围

in-scope 文档分两组：

- **全量组（`full`）**：高频、主题宽的文档。两个引擎都覆盖，并参与 baseline 计算。
...
合并两个引擎的待改项：先按「同文档同位置」去重（两个引擎常撞同一项，如供应商列表），再逐项归为「措辞/结构」「事实项」或「新能力」。
完成判据：每项都已去重并归类。
