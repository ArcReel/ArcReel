...
- 存在非终态 issue → 向用户列出批次标识与终态/在途分布，三选一：**接管**（走下方 §2–§3）/ **重开**（弃现状回 SKILL.md 第一步）/ **忽略**（一切不动）
...
- `no-branch` 且路线为 codex：codex 后台任务不随 lead 会话终止而死。先在对应 worktree 跑 companion `status --all` 分三态处置：已完成 → 按 SKILL.md「实现路线与模型」以 `result` 交付并核验，不重跑；在途且有进展 → 等待其完成；失败或停滞 → 按 SKILL.md 健康检查节的替补处置执行
