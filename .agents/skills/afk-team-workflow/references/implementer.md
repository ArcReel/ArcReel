...
5. **质量门**：`uv run pytest`（全量）、`uv run ruff check . && uv run ruff format .`、`uv run basedpyright`；改动涉及前端则加 `cd frontend && pnpm lint && pnpm check`。判定口径是**无新增失败**：遇到疑似与本 issue 无关的失败，先在未改动的 main 工作区验证能否复现——能复现则报 lead 备案后放行；不能复现即本次改动引入，必须修复
