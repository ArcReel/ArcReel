...
开工前检查 `.afk/` 是否存在**缺少 `closed` 收尾行**的账本（例如 `grep -L '"kind":"closed"' .afk/*.jsonl`）。若有，则上一会话的 lead 可能中途终止——读 [references/recovery.md](references/recovery.md) 按接管流程处理，不要当作全新批次直接覆盖。若无，照常继续。
...
```bash
bash .agents/skills/afk-team-workflow/scripts/ledger.sh <batch-id> <kind> [--issue N] [--pr M] [--detail "..."]
```
