...
开工前检查 `.afk/` 是否存在**末条事件不是 `closed`** 的账本。判据是末条而非全文：`closed` 只在末条才表示收尾，中段出现（其后又有追加）不算，故 `grep` 全文会误判。逐文件取末条比对，例如 `for f in .afk/*.jsonl; do tail -n1 "$f" | grep -q '"kind":"closed"' || echo "$f"; done` 列出未收尾账本。若有，则上一会话的 lead 可能中途终止——读 [references/recovery.md](references/recovery.md) 按接管流程处理，不要当作全新批次直接覆盖。若无，照常继续。
...
4. **主动请求一次性前置授权**：向用户明确提出"本批所有 PR 的合并是否预先批准"，连同流程将自动执行的动作边界（修改 triage 标签、PR 转 draft、在 PRD 发 QA 验收 comment；不自行创建新 issue，立项永远源于用户指令）。这是本流程唯一的同步确认点；前置授权在此落入 lead 的 transcript，后续合并不再逐笔请示。合并是 lead 的低频高危动作，**口头预批即可、不写入 settings 放行**（遵 memory `feedback_verbal_approval_no_settings_edit`）；teammate 的高频低危动作走持久 settings，见「授权前置设置」
...
- **合并**是 lead 的高危低频动作，走第二步的口头前置授权，**不写入 settings**。
- **teammate 的高频低危动作**（顶层评论、行内回复、建 PR、push / rebase、改标签）走持久化 `.claude/settings.local.json` allow，由 teammate 继承。推荐 allow 至少包含：
  - `Bash(gh pr comment:*)` / `Bash(gh issue comment:*)`——PR / QA / 请示评论
  - `Bash(gh api repos/*/pulls/*/comments/*/replies:*)`——行内 review 回复（receiving-code-review 回评论线程用 `gh api .../replies`，不走 `gh pr comment`）
  - `Bash(gh pr create:*)` / `Bash(gh pr ready:*)` / `Bash(gh issue edit:*)` / `Bash(git push:*)` / `Bash(git rebase:*)` / `Bash(git checkout:*)`——建 PR、转 draft、改标签、推送与 rebase

`.claude/settings.local.json` 是 gitignored 的本地配置，skill 无法修改——以上为供用户开工前自行就位的建议清单。

**边界说明**：skill 无法保证 permission classifier 的判定，也无法保证这些 allow 已就位；缺失某条时，对应动作会在运行中被拦截、转人工。此外，超长会话经上下文压缩后，第二步落入 transcript 的口头前置授权可能丢失——届时就该笔合并重新征求一次，不默认仍然预批。
