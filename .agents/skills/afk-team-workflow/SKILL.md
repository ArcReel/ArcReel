...
先从本 skill 目录运行 batch-poll，取得批次的机械底图：展开 Spec 子 issue、解析依赖图、给出每个 issue 的远端落点（标签、`blocked_by`、分支/PR 状态、`stage_hint`）：

```bash
bash scripts/batch-poll.sh --spec <N>      # Spec 编号：展开其 GitHub 子 issue
bash scripts/batch-poll.sh --issues 1,2,3  # 跨 Spec 的显式 issue 集
```
