...
3. **不收敛**（`round_estimate` ≥ 5，每轮都是新出现的小意见，没有单一争点）：先判断成因再选处置——①注意力漂移：令 looper 把本轮修复交由子代理在干净上下文中执行，自己继续负责轮询；②防御堆积：令 looper 从严执行可达性门槛——指不出触发路径的防御类意见一律驳回，指得出的照常修复；驳回项按 follow-up 候选记入 handoff，交清尾轮分拣；③issue 拆分过粗：按 needs-human 搁置并转呈。裁决 append 账本 `decision`
