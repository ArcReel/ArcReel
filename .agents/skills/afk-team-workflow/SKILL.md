...
- 一次只合一笔。合并前核对 review-looper 的达标报告，并确认 `gh pr view <M> --json mergeable` 为 CLEAN
...
2. **可实证驳回的重复意见**：同一主题已有 pushback 在案（跑过测试、查过文档），又被同一 reviewer 重复提出——按收敛兜底收口，不算 reviewer 冲突；浮现出值得升级 ADR 的原则时记入收尾转呈，不当场写 ADR
