...
async def _declared[ResultT](
    declaration: ToolDeclaration[Any, ResultT], ctx: ToolContext, args: dict[str, Any]
) -> ToolOutcome[ResultT]:
    """经声明的共享入口调用：请求校验、迁移阻断策略与 handler 同两宿主一致。"""
    return await invoke_declaration(declaration, args, ctx.scope, ctx.caller, tool_services(ctx))
...
async def _patch(
    ctx: ToolContext, operations: list[dict[str, Any]], *, script: str = "episode_1.json"
) -> ToolOutcome[ScriptPatchResult]:
    return await _declared(
...
class TestPatchEpisodeScript:
    ...
    async def test_empty_operations_rejected(self, ctx: ToolContext) -> None:
    ...
    async def test_prompt_change_requires_regeneration(self, ctx: ToolContext) -> None:
    ...
    async def test_non_prompt_change_requires_no_regeneration(self, ctx: ToolContext) -> None:
...
class TestPatchProject:
    ...
    async def test_modify_existing_character_merges_fields(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"table": "characters", "entries": {"李白": {"description": "剑客"}}})
        out = await _declared(
            PATCH_PROJECT,
            ctx,
    ...
    async def test_unknown_table_errors(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"table": "weapons", "entries": {"剑": {"description": "x"}}})
        assert out.problem is not None
    ...
    async def test_empty_derivative_table_reports_noop(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"table": "characters", "entries": {"李白": {"description": "白衣剑客"}}})
...
class TestPatchProjectSettings:
    ...
    async def test_set_episode_target_units(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": 1000}})
        assert out.problem is None
    ...
    async def test_clear_episode_target_units(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": 1000}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": None}})
        assert out.problem is None
    ...
    async def test_noop_when_same_value(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": 800}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": 800}})
        assert out.problem is None
        assert "无变更" in _said(out)

    async def test_non_whitelist_field_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"arbitrary_field": 1}})
        assert out.problem is not None
    ...
    async def test_route_fields_not_patchable_via_settings(self, ctx: ToolContext, field: str, value: Any) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {field: value}})
        assert out.problem is not None
    ...
    async def test_set_source_language_allowed_values(self, ctx: ToolContext, lang: str) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"source_language": lang}})
        assert out.problem is None
        assert ctx.pm.load_project("demo")["source_language"] == lang

    async def test_clear_source_language(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"source_language": "en"}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"source_language": None}})
        assert out.problem is None
    ...
    async def test_invalid_source_language_rejected(self, ctx: ToolContext, bad: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"source_language": bad}})
        assert out.problem is not None
        assert "source_language" not in ctx.pm.load_project("demo")
    ...
    async def test_set_character_voice_binding_allowed_values(self, ctx: ToolContext, binding: str) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"character_voice_binding": binding}})
        assert out.problem is None
    ...
    async def test_clear_character_voice_binding(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"character_voice_binding": "reference_audio"}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"character_voice_binding": None}})
        assert out.problem is None
    ...
    async def test_invalid_character_voice_binding_rejected(self, ctx: ToolContext, bad: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"character_voice_binding": bad}})
        assert out.problem is not None
        assert "character_voice_binding" not in ctx.pm.load_project("demo")
    ...
    async def test_invalid_value_rejected(self, ctx: ToolContext, bad_value: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_units": bad_value}})
        assert out.problem is not None
        assert "episode_target_units" not in ctx.pm.load_project("demo")
    ...
    async def test_positive_int_setting_accepts_digit_string(self, ctx: ToolContext, key: str) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {key: "10"}})
        assert out.problem is None
    ...
    async def test_set_and_clear_planning_overrides(self, ctx: ToolContext, key: str) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {key: 12}})
        assert out.problem is None
        assert ctx.pm.load_project("demo")[key] == 12
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {key: None}})
        assert out.problem is None
    ...
    async def test_invalid_planning_override_rejected(self, ctx: ToolContext, key: str, bad_value: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {key: bad_value}})
        assert out.problem is not None
        assert key not in ctx.pm.load_project("demo")
    ...
    async def test_neither_table_nor_settings_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {})
        assert out.problem is not None

    async def test_empty_settings_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {}})
        assert out.problem is not None
...
class TestPatchProjectNarrationSettings:
    ...
    async def test_set_narration_voice(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": "Ethan"}})
        assert out.problem is None
        assert ctx.pm.load_project("demo")["narration_voice"] == "Ethan"
        assert "已更新" in _said(out)

    async def test_modify_narration_voice(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": "Ethan"}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": "Cherry"}})
        assert out.problem is None
        assert ctx.pm.load_project("demo")["narration_voice"] == "Cherry"

    async def test_clear_narration_voice(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": "Ethan"}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": None}})
        assert out.problem is None
    ...
    async def test_invalid_narration_voice_rejected(self, ctx: ToolContext, bad: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_voice": bad}})
        assert out.problem is not None
        assert "narration_voice" not in ctx.pm.load_project("demo")

    @pytest.mark.parametrize("speed", [1.2, 0.5, 2, 1])
    async def test_set_narration_speed(self, ctx: ToolContext, speed: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_speed": speed}})
        assert out.problem is None
    ...
    async def test_narration_speed_accepts_numeric_string(self, ctx: ToolContext) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_speed": "1.5"}})
        assert out.problem is None
        assert ctx.pm.load_project("demo")["narration_speed"] == 1.5

    async def test_clear_narration_speed(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_speed": 1.2}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_speed": None}})
        assert out.problem is None
    ...
    async def test_invalid_narration_speed_rejected(self, ctx: ToolContext, bad: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"narration_speed": bad}})
        assert out.problem is not None
...
class TestPatchProjectOverview:
    ...
    async def test_merge_preserves_untouched_fields(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"overview": {"synopsis": "原始梗概", "genre": "原题材"}})
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {"genre": "悬疑"}})
        assert out.problem is None
        ov = ctx.pm.load_project("demo")["overview"]
        assert ov["genre"] == "悬疑"
    ...
    async def test_creates_overview_when_absent(self, ctx: ToolContext) -> None:
        ...
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {"synopsis": "新建概述"}})
        assert out.problem is None
    ...
    async def test_non_whitelist_key_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {"title": "x"}})
        assert out.problem is not None
        assert "title" not in ctx.pm.load_project("demo").get("overview", {})

    async def test_non_string_value_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {"synopsis": 1}})
        assert out.problem is not None
        assert "overview" not in ctx.pm.load_project("demo")

    async def test_empty_overview_rejected(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {}})
        assert out.problem is not None

    async def test_noop_when_same_value(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"overview": {"synopsis": "同值"}})
        out = await _declared(PATCH_PROJECT, ctx, {"overview": {"synopsis": "同值"}})
        assert out.problem is None
        assert "无变更" in _said(out)
...
class TestPatchProjectEpisodeTargetDurationSetting:
    ...
    async def test_set_episode_target_duration(self, ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_duration": 120}})
        assert out.problem is None
    ...
    async def test_clear_episode_target_duration(self, ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_duration": 120}})
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_duration": None}})
        assert out.problem is None
    ...
    async def test_out_of_range_or_dirty_value_rejected(self, ctx: ToolContext, bad: Any) -> None:
        out = await _declared(PATCH_PROJECT, ctx, {"settings": {"episode_target_duration": bad}})
        assert out.problem is not None
        assert "episode_target_duration" not in ctx.pm.load_project("demo")

    async def test_rejected_on_ad_project(self, ad_ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ad_ctx, {"settings": {"episode_target_duration": 120}})
        assert out.problem is not None
...
class TestPatchProjectBriefSetting:
    ...
    async def test_set_brief_on_ad_project(self, ad_ctx: ToolContext) -> None:
        out = await _declared(PATCH_PROJECT, ad_ctx, {"settings": {"brief": "突出 3 秒速干卖点"}})
        assert out.problem is None
    ...
    async def test_clear_brief_on_ad_project(self, ad_ctx: ToolContext) -> None:
        await _declared(PATCH_PROJECT, ad_ctx, {"settings": {"brief": "x"}})
        out = await _declared(PATCH_PROJECT, ad_ctx, {"settings": {"brief": None}})
        assert out.problem is None
...
class TestRenameAssetTool:
    ...
    async def test_missing_old_name_error_hints_idempotency(self, rename_ctx: ToolContext) -> None:
        await _declared(RENAME_ASSET, rename_ctx, {"table": "characters", "old_name": "角色A", "new_name": "主角甲"})
        out = await _declared(
            RENAME_ASSET, rename_ctx, {"table": "characters", "old_name": "角色A", "new_name": "主角甲"}
        )
        assert out.problem is not None
