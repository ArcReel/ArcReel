...
datetime，跨 session 的 finish/finalize 中 aware(finished_at) - naive(row.started_at)
相减抛 TypeError 被兜底成 duration_ms=0——这是 SQLite 后端的现行真实行为，矩阵按现状
锁定（PostgreSQL 下为真实时长，不在本矩阵口径内）。created_at / updated_at 由 ORM 列
默认绑定真实时钟、非记账语义字段，仅断言为 datetime。
"""
...
def _expected_row(**overrides: Any) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "started_at": T0,
        "finished_at": T1,
        # SQLite 现行行为：跨 session 时 aware - naive 相减抛 TypeError 兜底成 0（见模块 docstring）
        "duration_ms": 0,
        "retry_count": 0,
        "cost_amount": 0.0,
        "currency": "USD",
