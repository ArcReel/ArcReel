...
async def test_returns_none_when_ffmpeg_missing(tmp_path: Path):
    ...
    with (
        patch("lib.infra.thumbnail.shutil.which", return_value=None),
        patch("lib.infra.thumbnail.asyncio.create_subprocess_exec") as spawn,
    ):
...
async def test_ffmpeg_available_attempts_extraction(tmp_path: Path):
    ...
    with (
        patch("lib.infra.thumbnail.shutil.which", return_value="/usr/bin/ffmpeg"),
        patch(
            "lib.infra.thumbnail.asyncio.create_subprocess_exec",
...
async def test_last_frame_returns_none_when_only_ffprobe_missing(tmp_path: Path):
    ...
    with (
        patch("lib.infra.thumbnail.shutil.which", side_effect=_which),
        patch("lib.infra.thumbnail.asyncio.create_subprocess_exec") as spawn,
    ):
        result = await thumbnail_module.extract_video_last_frame(video, out)
...
async def test_last_frame_falls_back_to_count_frames(tmp_path: Path):
    ...
    with (
        patch("lib.infra.thumbnail.shutil.which", side_effect=_which),
        patch("lib.infra.thumbnail.asyncio.create_subprocess_exec", side_effect=_spawn),
    ):
        result = await thumbnail_module.extract_video_last_frame(video, out)
