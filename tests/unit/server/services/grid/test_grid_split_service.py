...
class TestApplyGridSplit:
    async def test_split_writes_cells_and_versions(self, project_with_script, grid_with_image):
        ...
        with (
            ...
            patch(
                "server.services.tasks.generation_tasks.emit_generation_success_batch", return_value={"grids/x.png": 1}
            ),
        ):
    ...
    async def test_split_emits_grid_split_event(self, project_with_script, grid_with_image):
        ...
        with (
            patch("server.services.grid.grid_split.get_project_manager", return_value=pm),
            patch(
                "server.services.tasks.generation_tasks.emit_generation_success_batch", return_value={"a": 1}
            ) as emit,
        ):
            result = await apply_grid_split("test-project", grid)
    ...
    async def test_source_claim_change_rolls_back_the_whole_split(self, project_with_script, grid_with_image):
        ...
        from server.services.grid import grid_split as grid_split_module
    ...
    async def test_reference_change_while_building_cell_basis_rolls_back_the_whole_split(
        ...
        with (
            patch("server.services.grid.grid_split.get_project_manager", return_value=pm),
            patch(
                "server.services.grid.grid_split.build_grid_member_storyboard_visual_basis",
    ...
    async def test_stale_source_composite_change_rolls_back_the_whole_split(
        ...
        with (
            patch("server.services.grid.grid_split.get_project_manager", return_value=pm),
            patch(
                "server.services.grid.grid_split.build_stale_grid_member_storyboard_visual_basis",
    ...
    async def test_script_resolution_failure_removes_the_composite_snapshot(
        ...
        with (
            patch("server.services.grid.grid_split.get_project_manager", return_value=pm),
            patch("lib.script.script_editor.resolve_items", side_effect=RuntimeError("script resolution failed")),
