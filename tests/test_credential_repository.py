"""ProviderCredential Repository tests."""
...
class TestCredentialRepository:
    async def test_create_and_list(self, session: AsyncSession):
        ...
        cred = await repo.create(provider="gemini-aistudio", name="TestKey", api_key="AIza-test")
        await session.flush()
        creds = await repo.list_by_provider("gemini-aistudio")
        assert len(creds) == 1
        assert creds[0].name == "TestKey"
    ...
    async def test_first_credential_is_active(self, session: AsyncSession):
        repo = CredentialRepository(session)
        cred = await repo.create(provider="gemini-aistudio", name="First", api_key="AIza-1")
        await session.flush()
        assert cred.is_active is True

    async def test_second_credential_is_not_active(self, session: AsyncSession):
        repo = CredentialRepository(session)
        await repo.create(provider="gemini-aistudio", name="First", api_key="AIza-1")
        cred2 = await repo.create(provider="gemini-aistudio", name="Second", api_key="AIza-2")
        await session.flush()
        assert cred2.is_active is False

    async def test_activate(self, session: AsyncSession):
        repo = CredentialRepository(session)
        c1 = await repo.create(provider="gemini-aistudio", name="First", api_key="AIza-1")
        c2 = await repo.create(provider="gemini-aistudio", name="Second", api_key="AIza-2")
        await session.flush()
    ...
    async def test_update(self, session: AsyncSession):
        repo = CredentialRepository(session)
        c = await repo.create(provider="gemini-aistudio", name="OldName", api_key="AIza-old")
        await session.flush()
        await repo.update(c.id, name="NewName", api_key="AIza-new")
        await session.flush()
