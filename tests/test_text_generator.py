...
class TestTextGenerator:
    ...
    async def test_generate_records_usage_on_failure(self, tracker):
        backend = _make_backend()
        backend.generate = AsyncMock(side_effect=RuntimeError("API timeout"))
        gen = TextGenerator(backend, tracker)
        ...
        assert "API timeout" in item["error_message"]

    async def test_generate_without_project_name(self, tracker):
        backend = _make_backend()
        gen = TextGenerator(backend, tracker)

        result = await gen.generate(TextGenerationRequest(prompt="toolbox call"))
