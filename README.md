...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=ArcReel)** 是一个全模态 AI 推理平台，用一套 OpenAI 兼容 API 同时覆盖 ArcReel 所需的文本（DeepSeek、Qwen、GLM、Kimi、MiniMax…）、图像（Seedream、Qwen-Image、GPT Image…）与视频（Seedance、Wan、Kling、Veo…）。作为「自定义供应商」一键接入即可，无需分别对接多家厂商。
> 文本 · [coding plan](https://www.atlascloud.ai/console/coding-plan) ｜ 图像/视频模型 · [atlascloud.ai/models](https://www.atlascloud.ai/models)
...
#### 接入示例：Atlas Cloud（OpenAI 兼容，文本 + 图像 + 视频一站式）

[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=ArcReel) 是一个全模态 AI 推理平台，用一套 OpenAI 兼容 API 同时提供 LLM、图像、视频能力，正好覆盖 ArcReel 的文本/图片/视频三类后端。在设置页「自定义供应商」中按以下信息接入：

| 字段 | 值 |
|------|----|
| 类型 | OpenAI 兼容 |
| Base URL | `https://api.atlascloud.ai/v1` |
| API Key | 你的 Atlas Cloud API Key（[控制台](https://www.atlascloud.ai/console)获取） |

填好后 ArcReel 会调用 `/v1/models` 自动发现模型并分配媒体类型。常用模型参考：

- **文本**：`deepseek-ai/deepseek-v4-pro`、`Qwen/Qwen3-Next-80B-A3B-Instruct`、`zai-org/glm-5`、`moonshotai/kimi-k2.6`、`MiniMaxAI/MiniMax-M2`（`deepseek-v4-pro` 为推理模型，请把 `max_tokens` 留足 ≥ 512）
- **图像**：`openai/gpt-image-2/text-to-image`、`bytedance/seedream-v5.0-lite`、`qwen/qwen-image-2.0/text-to-image`、`google/nano-banana-pro/text-to-image`
- **视频**：`bytedance/seedance-2.0/text-to-video`、`alibaba/wan-2.7/text-to-video`、`kwaivgi/kling-v3.0-pro/text-to-video`、`google/veo3.1/text-to-video`

完整模型清单见 [atlascloud.ai/models](https://www.atlascloud.ai/models)。
