...
<td width="20%" align="center">
<h3>🎨 多供应商图像生成</h3>
<strong>Gemini</strong>、<strong>火山方舟</strong>、<strong>Grok</strong>、<strong>OpenAI</strong>、<strong>Vidu</strong>、<strong>阿里百炼</strong>、<strong>MiniMax</strong>、<strong>可灵</strong> 及自定义供应商，角色设计图帮助提升角色一致性，线索追踪增强道具/场景跨镜连贯
</td>
<td width="20%" align="center">
<h3>🎬 多供应商视频生成</h3>
...
- **自定义供应商** — 接入实现受支持发现协议和调用端点的 OpenAI 兼容 / Google 兼容 API，自动发现或手动登记模型；实际能力取决于模型配置、调用端点与上游实现
...
- **旁白配音（TTS）** — 说书模式可在设置页配置音色与语速，分镜逐段试听、一键补齐全集，智能体一句话即可生成全集；支持阿里百炼 Qwen3 TTS 与 OpenAI 兼容 TTS，导出剪映草稿自带逐段旁白音轨
...
除预置供应商外，可接入实现 ArcReel 所支持发现协议和调用端点的 **OpenAI 兼容** 或 **Google 兼容** API：

- 在设置页添加自定义供应商，填入 Base URL 和 API Key
- 按所选发现协议获取可用模型，并根据模型 ID 推断调用端点和媒体类型（文本/图片/视频/语音）；保存前可手动调整推断结果
- 可参与全局/项目级切换、费用追踪和版本管理；实际生成能力取决于模型配置、调用端点与上游实现
...
- ❓ [常见问题](docs/FAQ.md) — 安装、配置、制作与故障排查
