...
- **提交与 PR**：标题用 conventional-commit 前缀（`feat`/`fix`/`chore`/`refactor` + `(scope)`）。squash 合并下 PR 标题即 changelog 条目——写用户可感知的收益、范围词用产品术语，不写实现术语（status_code、内部类名等）且诚实限定范围；改正时只改标题不 amend commit
