...
- profile 同步由 `lib/profile_manifest.py` 通过 manifest + sha256 驱动，仅把声明过且校验通过的文件复制到各项目的 `.claude/` 与 CLAUDE.md，运行时从项目侧加载。同步不覆盖用户改过的项目侧文件（此后也不再随 profile 升级），共享配置一律改源目录
