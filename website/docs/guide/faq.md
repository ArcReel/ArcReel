...
## 首次配置与 Agent {#setup-and-assistant}

### 开始制作前需要配置哪些凭据？ {#required-credentials}

ArcReel Agent（智能体）与内容生成使用两套独立配置：

- **Agent 供应商**负责对话、分析原文和编排制作任务，需要一个可用且已激活的 Agent 凭据。
...
### 怎样让 Agent 开始或继续制作？ {#start-or-resume-assistant}
...
### 项目模式以及分镜和参考生视频路线怎样选择？创建后能改吗？ {#choose-mode-and-route}

创建时需要分别确定两个维度：

- **内容模式**：旁白/解说、剧情演绎或广告/短片，决定剧本结构和制作流程。
- **视频生成路线**：分镜路线或参考生视频路线，决定视频使用分镜图还是资产参考图。

内容模式和视频生成路线创建后不可更改。多宫格分镜不是第三条路线，而是分镜路线中的出图方式；广告/短片项目不支持多宫格分镜。创建前请先用短样本确认路线是否符合预期。

- **分镜路线**：剧本 → 角色/场景/道具资产图 → 分镜图 → 视频。生成视频前必须有对应分镜图。
- **多宫格分镜**：仍属于分镜路线，是路线内的出图方式。系统先生成一张或多张多宫格分镜并切分成各镜头的起始分镜，再由分镜图生成视频。
- **参考生视频路线**：跳过分镜图，直接使用剧本引用的角色、场景和道具资产图作为视频参考。
...
### 为什么下一阶段不能进入，或 Agent 没有继续执行？ {#stage-blocked}

通常是前置审核或资产尚未完成：

1. 旁白/解说和剧情演绎项目检查剧本预处理结果是否已确认；确认后再次编辑需要重新确认。广告/短片项目没有这一步。
2. 检查角色、场景和道具是否只有定义而没有已生成的资产图。
3. 分镜路线检查目标镜头是否已有分镜图。
4. 参考生视频路线中，旁白/解说和剧情演绎项目检查被引用资产的资产图是否齐全；广告/短片项目至少确认商品原图已上传，缺少参考图虽可能不阻塞任务，但会降低商品保真度。
...
生成分镜时，ArcReel 会把该镜头引用的角色、场景和道具资产图作为参考；分镜路线的视频再以分镜图作为首帧，参考生视频路线则直接使用资产图。生成模型仍无法保证逐帧绝对一致。
