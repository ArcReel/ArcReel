...
### 为什么连通性检查成功，实际生成仍然失败？ {#connection-ok-generation-fails}
...
1. 旁白/解说和剧情演绎项目检查脚本规划结果是否已确认；确认后再次编辑需要重新确认。广告/短片项目没有这一步。
...
视频模型自带人声、角色参考音频和独立旁白配音（TTS）是不同能力。角色声音默认按角色的声音描述作提示词软约束，不能保证跨片段音色完全一致；要让视频模型直接复用角色原声，需在项目设置里把「角色声音绑定方式」改为「按参考音频」，同时项目走参考生视频、所选视频模型明确支持参考音频，并为每个有台词的角色配好参考音频。分镜图生视频不走参考音频通道。
...
1. 用少量分镜测试目标模型的画风、参考图、时长和审核限制。
2. 先确定角色、场景和道具资产图，再批量生成分镜。
3. 让相邻分镜保持地点、时间、服装和角色描述一致。
4. 只引用当前分镜真正需要的资产，避免参考图过多稀释约束。
