...
## 使用 ArcReel 前需要了解哪些能力边界？ {#capability-boundaries}
...
- Windows 原生环境可以运行部分基础流程，但智能体沙箱等 POSIX 能力会降级；优先使用 Linux、macOS、WSL2 或 Docker。
...
Windows 原生环境可以运行项目创建等基础流程，但智能体沙箱会降级，生产部署仍建议使用 WSL2 或 Docker Desktop。Linux/macOS 本地运行需要可用的系统沙箱工具，详见[部署补充说明](../ops/deployment.md)。
...
Web UI 的项目 ZIP 适合迁移单个项目，但不包含全局供应商配置、账号配置、任务记录、费用记录或智能体会话，因此不能代替全站备份。

## 首次配置与智能体 {#setup-and-assistant}

### 开始制作前需要配置哪些凭据？ {#required-credentials}
...
只配置智能体凭据不会自动获得生图或视频能力；只配置生成供应商也无法启动项目内的 AI 智能体。项目级生成模型设置优先于全局默认，排障时要确认项目实际选中的供应商和模型。

预置生成供应商和智能体供应商可保存多条凭据，但运行时只使用当前标记为“激活”的一条，不会自动轮换；测试某条凭据也不等于已经激活它。每个自定义供应商当前保存一条 API Key。

### 怎样让 AI 智能体开始或继续制作？ {#start-or-resume-assistant}

在项目右侧打开 AI 智能体，直接说明“开始制作”或“继续制作”即可。智能体会检查项目当前状态，从尚未完成的阶段继续，并在需要审核剧本、资产或分镜时等待确认。

如果智能体提示未登录、无法启动或没有可用模型，先到设置页确认智能体凭据已经保存、消息调用探测成功且处于激活状态。模型发现告警不一定阻断消息调用，但应核对实际模型 ID。无需在智能体面板另行登录 Claude 网页账号。

### 出现 `Failed to start Claude Code`、启动超时或 `Control request timeout` 怎么办？ {#assistant-startup-failure}

这些提示表示智能体进程没有正常启动或未能及时完成初始化，不等同于剧本内容错误。依次检查：

1. 智能体凭据是否已激活，模型 ID 是否与服务端实际开放的 ID 完全一致。
2. Docker 容器或服务器能否访问智能体 API，代理、DNS 和 TLS 是否正常。
3. Docker/WSL/本地环境的智能体沙箱是否通过启动检查。
...
### ArcReel 支持哪些供应商和模型？ {#supported-providers}
...
### Base URL 应该怎样填写？ {#base-url-format}
...
### 怎样接入自定义供应商？ {#custom-provider-setup}
...
### 获取模型列表失败，或者出现 `model_not_found` 怎么办？ {#model-list-failure}
...
### 为什么连接测试成功，实际生成仍然失败？ {#connection-ok-generation-fails}
...
### 遇到 401/403、429 或网络超时怎么办？ {#auth-rate-limit-timeout}
...
## 项目流程与任务 {#project-workflow-and-tasks}

### 项目模式以及分镜和参考生视频怎样选择？创建后能改吗？ {#choose-mode-and-route}
...
创作类型和生成模式创建后不可更改。多宫格分镜不是第三条路线，而是分镜图生视频中的出图方式；广告/短片项目不支持多宫格分镜。创建前请先用短样本确认路线是否符合预期。
...
### 为什么下一阶段不能进入，或智能体没有继续执行？ {#stage-blocked}

通常是前置审核或资产尚未完成：

1. 旁白/解说和剧情演绎项目检查内容整理结果是否已确认；确认后再次编辑需要重新确认。广告/短片项目没有这一步。
2. 检查角色、场景和道具是否只有定义而没有已生成的资产图。
3. 分镜图生视频检查目标分镜是否已有分镜图。
4. 参考生视频中，旁白/解说和剧情演绎项目检查被引用资产的资产图是否齐全；广告/短片项目至少确认商品原图已上传，缺少参考图虽可能不阻塞任务，但会降低商品保真度。
5. 展开任务面板，确认是否仍有排队、运行或失败的任务。

不要仅根据顶部阶段编号判断完成状态；左侧资产、剧集状态和任务错误会提供更具体的缺失项。

### 任务排队、运行、失败、取消或服务重启后应该怎么办？ {#task-states}
...
### 首帧和尾帧怎样使用？ {#first-and-last-frame}

分镜图生视频中的分镜图就是视频首帧。每个分镜还可以选择或上传尾帧，但只有当前视频模型明确支持尾帧时才能生成；不支持时系统会拒绝请求，而不是静默忽略。

参考生视频没有独立尾帧设置。更换分镜或尾帧后，已有视频不会自动更新，需要重新生成对应视频。

### 为什么场景资产图里出现了人物？ {#people-in-scene-images}

场景资产图的提示词会要求画面中不出现人物，但图片模型仍可能不完全遵循约束。此时重新生成场景图，或使用指令式图片编辑移除人物。

场景资产图与剧情分镜图不同：场景资产图应聚焦环境，分镜图则会根据剧情显示人物。

### ArcReel 支持配音吗？ {#voice-over-support}

当前 Web UI 的独立旁白 TTS 仅对旁白/解说开放，可逐段试听或生成全集旁白，并随剪映草稿导出。语音供应商、音色和语速可在全局或项目设置中配置；部分模型不支持语速调整。

视频模型自带人声、角色参考音频和独立旁白 TTS 是不同能力。角色参考音频目前用于参考生视频，并且所选视频模型必须明确支持参考音频；分镜图生视频及仅有声音描述时属于软约束，不能保证跨片段音色完全一致。

## 费用、数据与导出 {#cost-data-export}

### 在哪里查看费用和 Token？怎样控制成本？ {#view-and-control-cost}
...
### 项目 ZIP 与全站备份有什么区别？ {#project-zip-vs-full-backup}

项目 ZIP 用于导入、迁移单个项目，可以选择仅导出当前版本或包含完整版本历史。它不包含全局供应商设置、登录配置、任务与费用记录或智能体会话。

需要灾难恢复时，请使用前文的全站备份方式，同时保存项目目录、数据库与所需凭据。

### 剪映草稿为什么看不到，或者缺少片段？ {#jianying-draft-issues}

导出时选择与本地剪映匹配的 5.x 或 6+ 格式，将 ZIP 直接解压到剪映草稿目录，并重启剪映。草稿只包含已成功生成的视频片段；缺失片段应先回到 ArcReel 补生成再导出。

当前旁白/解说导出小说原文字幕，广告/短片导出口播字幕，剧情演绎导出台词与画外音字幕；旁白/解说还会带上已生成的旁白音轨。完整步骤见[剪映草稿导出指南](./jianying-export.md)。

### 有手机 App 吗？支持哪些平台？ {#mobile-app-support}
...
ArcReel 按 [AGPL-3.0](https://github.com/ArcReel/ArcReel/blob/main/LICENSE) 发布，并附带 [NOTICE](https://github.com/ArcReel/ArcReel/blob/main/NOTICE) 中的署名和修改声明要求。ArcReel 名称与 Logo 不在 AGPL-3.0 授权范围内。
