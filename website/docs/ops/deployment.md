...
- 改写宫格记录（`grids/`）与持久化呈现（`presentations/`）的迁移另备份被改写的记录文件。
...
- 绑定到非规范文件名（例如 `scripts/custom.json`）的集，正式脚本改名为 `scripts/episode_N.json` 并改绑，产物记录、宫格记录与版本记录里的剧本文件名随之更新。`scripts/episode_N.json` 上原本另有一份内容不同的文件时，它另存为同目录的 `episode_N.json.displaced-v14`，不会被自动清理；规范化结果与另存位置列在迁移报告的 `normalized_bindings` 里，改不过去的集（例如规范文件名已绑给另一集）列在跳过项里。有几种绑定无法规范化：同一个集号在 `episodes` 里出现多条，同一个剧本文件被绑给多集，绑定指到 `scripts/` 之外，改不过去而要原样保留的绑定带有目录段（如 `scripts/archive/custom.json`，反斜杠写法 `scripts/archive\custom.json` 同样算），以及绑定的文件读不出来或里面记的集号不是这一集。这些情况下该项目直接迁移失败并停在升级前的版本，剧本文件不被改动，失败记录写明出问题的集号与绑定；把 `project.json` 里重复的 `script_file` 改成各集自己的剧本、把越界或带目录段的绑定改成 `scripts/` 下的文件名、或修好那份剧本后重启即可继续。
