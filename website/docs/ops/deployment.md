...
- 绑定到非规范文件名（例如 `scripts/custom.json`）的集，正式脚本改名为 `scripts/episode_N.json` 并改绑，产物记录、宫格记录与版本记录里的剧本文件名随之更新。`scripts/episode_N.json` 上原本另有一份内容不同的文件时，它另存为同目录的 `episode_N.json.displaced-v14`，不会被自动清理；规范化结果与另存位置列在迁移报告的 `normalized_bindings` 里，改不过去的集（例如规范文件名已绑给另一集）列在跳过项里。
