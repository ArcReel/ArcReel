...
- 集的剧本绑定只认规范文件名 `scripts/episode_N.json`。某一集绑到别的名字（例如 `scripts/custom.json`），或同一个集号在 `episodes` 里出现多条时，该项目直接迁移失败并停在升级前的版本，项目目录一个字节都不被改动，失败记录写明出问题的集号与绑定。把那份剧本改名为 `scripts/episode_N.json`、把 `project.json` 里的 `script_file` 改成同一个名字（重复的集号条目只留一条）后重启即可继续。
