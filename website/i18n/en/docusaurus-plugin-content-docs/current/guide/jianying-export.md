---
id: jianying-export
title: CapCut Draft Export
sidebar_position: 4
---

# CapCut Draft Export {#jianying-export}

Export video clips generated by ArcReel as a CapCut draft for each episode, then open the draft directly in CapCut Desktop for further editing—adjust the pacing, add subtitles, transitions, voice-over, and more.

## Prerequisites {#prerequisites}

- Video clips have been generated for at least one episode in ArcReel
- **CapCut Desktop** (5.x or 6+) is installed locally

## Steps {#steps}

### 1. Locate the CapCut draft directory {#locate-draft-directory}

Before exporting, you need to know where local CapCut drafts are stored.
...
3. Select **Export to CapCut Draft**
...
Extract the downloaded ZIP file into the CapCut draft directory entered above. The extracted structure is as follows:
...
### 5. Open in CapCut {#open-in-jianying}

1. Open (or restart) CapCut Desktop
...
- **Subtitle track**: Character dialogue and voice-over from the screenplay are added automatically; the style, position, and timing can be adjusted further in CapCut
...
### The exported draft does not appear in CapCut {#draft-not-visible}
...
- Try restarting CapCut

### What if the versions do not match? {#version-mismatch}

The CapCut version selected during export must match the locally installed version:
- CapCut 6.0 or later → select **6.0+**
- CapCut 5.x → select **5.x**
