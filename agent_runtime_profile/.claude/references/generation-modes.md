...
| generation_mode | content_mode | 数据主结构 | 预处理子智能体 | step1 中间文件 | 脚本 schema | 视觉参考来源 |
|---|---|---|---|---|---|---|
...
预处理该 dispatch 哪个子智能体，一律读 `mcp__arcreel__get_workflow_plan` 的 `steps[]` 与
`next_action`（`prepare_step1` 的 `next_action.args.preprocessor` 就是权威的预处理子智能体名）。
