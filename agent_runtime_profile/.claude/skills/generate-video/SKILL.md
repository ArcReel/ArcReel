...
| 单场景 | `mcp__arcreel__generate_video_scene({"script": "episode_1.json", "scene_id": "E1S1"})` |
...
| 旧脚本（已删除） | 新 tool |
|---|---|
| `python ... generate_video.py episode_N.json` | `mcp__arcreel__generate_video_episode({"script": ...})` |
| `... --resume` | 加 `"resume": true` |
| `... --scene ID` | `mcp__arcreel__generate_video_scene({..., "scene_id": "ID"})` |
| `... --scenes A,B,C` | `mcp__arcreel__generate_video_selected({..., "scene_ids": [...]})` |
| `... --all` | `mcp__arcreel__generate_video_all({...})` |
