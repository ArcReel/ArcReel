...
> 画面比例、时长、分辨率等规格由脚本根据 content_mode 自动设置，详见 `references/content-modes.md`。

## 命令行用法

```bash
# 标准模式：生成整集所有待处理场景（推荐）
cd projects/{project_name} && python ../../.claude/skills/generate-video/scripts/generate_video.py \
    --episode {N}
