...
2. 使用 Read 工具读取 `project.json`，确认 `title`、`creation_type`、`generation_mode` 字段（本 session 当前 creation_type 为 `narration`，创建后不可变更）
3. 请用户将小说文本放入 `source/`
...
- 否则（本项目 creation_type == narration）→ dispatch `split-narration-segments`（产出 `drafts/episode_{N}/step1_segments.json`）
