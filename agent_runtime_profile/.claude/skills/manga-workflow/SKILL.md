...
1. characters / scenes / props 全部为空？ → **阶段 1**
...
**触发**：project.json 中 characters / scenes / props 全部为空
...
三类任务互不依赖，**同时 dispatch 需要执行的 `generate-assets` subagent**，按类型分别调度：
...
只 dispatch 需要执行的分支。三个 subagent 全部返回后，合并摘要展示给用户，进入阶段间确认。
