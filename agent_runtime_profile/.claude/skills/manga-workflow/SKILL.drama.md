...
3. 目标集**当前组合对应的** step1 中间文件不存在？ → **阶段 3**。按 `effective_mode(project, episode)` × `content_mode` 三分支检查对应文件（注意 effective_mode 含集级 `episodes[i].generation_mode` 覆盖，不能只看项目顶层字段）：
   ...
   本项目 content_mode 固定为 drama（创建后不可变），故只会命中第 1 或第 3 分支，取决于该集的 effective_mode。只认当前组合对应的那一个文件：目录中出现**其他模式的 `step1_*` 文件**属残留，不作为阶段 3 已完成的依据。
...
1. 确定源文件：`source/_remaining.txt` 存在则使用，否则用原始小说文件，并核对 `project.json` 中 `source_language` 与源文实际语言。优先级：**用户显式配置 > 自动推断**（正常路径由 overview 生成自动落盘）；发现配置与源文实际语言不符时，**提醒用户（WARN）、说明后果并建议修正**（错误配置会使 peek 度量失真，对 en/vi 源按 zh 度量直接 count=0 失败），用户未修正时按显式配置继续，不阻塞流程。字段缺失或经用户确认有误时，走 `mcp__arcreel__patch_project({"settings": {"source_language": "en"|"vi"|"zh"}})` 写入或给 peek 显式传 `--language`
2. 解析目标字数（按 `source_language` 解读为阅读单位）：
   - 读 `project.json`，若已有 `episode_target_units` 字段 → 作为默认 target，向用户确认或调整
   - 字段缺失 + 用户当前对话已明确指定字数 → 使用对话值
   - 字段缺失 + 用户未指定 → AskUserQuestion 询问目标字数（选项形式与文案由 agent 按 `source_language` 实时判断）
   - 用户给出非临时性偏好时 → 调 `mcp__arcreel__patch_project({"settings": {"episode_target_units": N}})` 写回 project.json
3. 调用 `peek_split_point.py` 展示切分点附近上下文（peek 内部按 `source_language` 解读 --target）：
...
5. 用户确认后，先 dry run 验证（**注意**：split 的 --target 是字符级口径，必须用 peek 输出的 `split_target_chars`，不能直接复用阅读单位 target，否则混排/ASCII 占比高时锚点搜索可能错位甚至落空）：

   ```bash
   python .claude/skills/manage-project/scripts/split_episode.py --source {源文件} --episode {N} --target {peek 输出的 split_target_chars} --anchor "{锚点文本}" --dry-run
   ```
...
- 否则（本项目 content_mode == drama）→ dispatch `normalize-drama-script`（产出 `drafts/episode_{N}/step1_normalized_script.md`）
