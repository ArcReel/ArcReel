...
1. 确定源文件：`source/_remaining.txt` 存在则使用，否则用原始小说文件，并核对 `project.json` 中 `source_language` 与源文实际语言。优先级：**用户显式配置 > 自动推断**（正常路径由 overview 生成自动落盘）；发现配置与源文实际语言不符时，**提醒用户（WARN）并按显式配置继续，不阻塞流程**。字段缺失或经用户确认有误时，走 `mcp__arcreel__patch_project({"settings": {"source_language": "en"|"vi"|"zh"}})` 写入或给 peek 显式传 `--language`（缺失/错误放任不管时 peek 会按 zh 度量，对 en/vi 源直接 count=0 失败）
