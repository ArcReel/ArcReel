...
- 视频单元的 `text` 是一段自由文本，按引用语法写：台词与画外音记号可独占一行、也可跟在同一行的画面描述之后；不要写 `镜头N：` 之类的分段前缀。用 `@[名称]` 引用资产，名称必须逐字取自 `project.json` 三张表（不确定就 Read `project.json` 确认）；每个视频单元逐条 `@` 引用其发生地的场景，候选表里没有匹配该地点的场景时才改用文字描述地点并各单元保持同一句；不写外貌 / 服装 / 场景细节
