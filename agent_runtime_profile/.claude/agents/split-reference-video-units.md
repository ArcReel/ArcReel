...
- 视频单元的 `text` 是一段自由文本，按引用语法写：台词记号紧跟它所对应的那句动作，写在同一行末尾或紧接的下一行。用 `@[名称]` 引用资产，名称必须逐字取自 `project.json` 三张表（不确定就 Read `project.json` 确认）；每个视频单元逐条 `@` 引用其发生地的场景，候选表里没有匹配该地点的场景时才改用文字描述地点并各单元保持同一句；不写外貌 / 服装 / 场景细节
