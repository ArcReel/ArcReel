...
- creation_type 字段（narration 或 drama）
- generation_mode 字段（项目顶层唯一决定，创建后不可更改，不存在集级覆盖）
- characters、scenes、props 已有数据

使用 Glob 工具确认中间文件存在，按项目 `generation_mode` × `creation_type` 检查：
- generation_mode == reference_video（任一 creation_type）：`drafts/episode_{N}/step1_reference_units.json`（缺失时需先运行 `split-reference-video-units`）
- generation_mode == storyboard 且 creation_type == narration：`drafts/episode_{N}/step1_segments.json`（缺失时需先运行 `split-narration-segments`）
- generation_mode == storyboard 且 creation_type == drama：`drafts/episode_{N}/step1_normalized_script.json`（结构化内容；缺失时需先运行 `normalize-drama-script`。旧项目残留的 `step1_normalized_script.md` 是结构化前的自由文本稿，不算有效 step1，须重跑 normalize 产出 `.json`）
