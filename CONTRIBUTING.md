...
- **上站 `.md` 不能使用 JSX / import**：`website/docusaurus.config.ts` 设 `markdown.format: "detect"`，`.md` 按 CommonMark 解析而非 MDX，写入的 JSX 标签或 import 语句不会报编译错误，只会静默不渲染。需要 JSX 的页面改用 `.mdx`。
