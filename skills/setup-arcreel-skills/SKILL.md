...
3. 在当前工作区创建 `.arcreel/settings.json`，以 JSON 保存同一连接的 `mcp_url` 与 `api_key`；供 ArcReel skills 的本地脚本跨会话复用。两处已有配置指向不同实例时，请用户选择后再同步。
4. 宿主支持配置 MCP 工具调用超时时，把 `arcreel` 的超时设为至少 `600` 秒；视频生成可能超过默认超时。

## 验证

按宿主要求使配置生效，再无参数调用一次 ArcReel MCP 工具 `list_projects`。调用成功并返回结构化 `projects` 列表，且 `.arcreel/settings.json` 与生效的 MCP 配置指向同一实例，即完成接入；空列表也是有效结果。已有配置满足这两个条件时无需改写。失败时报告出错边界，不得暴露 API Key。
