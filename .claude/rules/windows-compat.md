...
- **POSIX-only `os` 常量**：`O_NOFOLLOW` / `O_DIRECTORY` 等逐个用 `getattr(os, "<常量名>", 0)` 取值。常量缺失时 `is_symlink()` 预检与随后的 `os.open()` 之间存在 TOCTOU 窗口，不能替代 `O_NOFOLLOW` 的原子保证：仅可信路径（如 `lib/agent/profile_manifest.py` 的项目锁）可只做预检；不可信路径须在打开后按 `st_dev` / `st_ino` 校验文件身份（参考 `lib/artifacts/artifact_manifest.py`），或在 Windows 上拒绝操作。
