...
- A link to the documentation site is another exception: `docs.arc-reel.com` destinations in `README.md` have no locale prefix because the default locale is unprefixed. `README.en.md` must route English readers to the English site, so give each `docs.arc-reel.com/...` destination an `/en/` prefix (`https://docs.arc-reel.com/guide/...` becomes `https://docs.arc-reel.com/en/guide/...`).
