...
清单条目相对当前内容的判定：current（与当前内容一致）/ stale（比当前内容旧：直接依赖已变但文件仍在）/ missing（应有而没有）/ blocked（读不出或依据不可用）。时效只追踪正式内容与直接上游产物，不纳入模型、分辨率、声音参数、供应商配置——配置变化由下一次显式生成生效，不把既有产物变 stale。**stale 仍可用**：可查看、照常参与成片与导出、不阻断工作流，也不是自动重生指令。
...
**补录（backfill）**：
项目 schema 迁移到产物清单时，把项目内既有正式产物按迁移那一刻的内容写入清单并直接认定为 current 的一次性动作；启动期扫描与归档导入共用同一入口，不持久化「来历不明 / legacy」标记，迁移后所有时效判断只走同一规则。补录失败是该项目的阻断性问题——原因原样透出、修复数据后重试，不是切回旧口径的理由。
...
**任务结果与产物时效独立**：
一次批量生成按本次请求穷尽为 succeeded ∪ failed ∪ blocked；产物则独立报告 current / stale / missing / blocked，两组状态互不推导——强制重生一个 current 产物失败时，任务是 failed，旧产物仍是 current。
