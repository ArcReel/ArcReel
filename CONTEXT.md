...
把一笔写了记账 key 的 image / video / audio 成本快照归到集与剧本条目的过程（text 调用与资产图不写 key：前者只进用量统计，后者按产物路径落项目级资产类型明细、拆不出类型的并入项目级未归属）。强证据是记账时冻结在调用记录 `segment_id` 列上的资源 ID，三种形态：剧本条目 ID（随骨架为 `segment_id` / `scene_id` / `shot_id` / `unit_id`，含 `_{子序号}` 后缀的子条目）、ad 参考生视频的派生 unit ID（认领后按成员镜头均摊）、宫格图 ID（`grid_*`，按 `generated_assets.grid_id` 反查认领后在覆盖的条目间均摊）。同一 (记账 key, 费用类型) 下的多条成本快照先按币种汇总成一份金额（币种各自分列），该份金额在一次估算内只被整份认领一次（去重防重复计入，不丢记录）；剧本认领不到的历史记账统一以「未归属（unassigned）」单列：项目级是全量汇总桶（收下每一笔未认领金额），能按 ID 前缀解析集号的另在对应集展示同一份（无前缀的宫格 ID 等只有项目级这份）——集级是按集视图不是新增支出，项目合计读项目级桶、不叠加集级。这些 ID 不保证唯一、不保证与所在集一致，属已知限制（见 `docs/adr/0053`）。
