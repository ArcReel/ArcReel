...
**分镜板（宫格 / grid）**：
把同一段落多个场景合并成一张 N×N 平方切分的联合大图一次生成、再切割成各场景起始分镜图的**分镜图生产方式**，核心价值在一次生成保证画风/角色一致。整图比例即项目视频比例，单格比例因平方切分与之相同，切格无裁切损失；档位阶梯收口在 `lib/grid/layout.py`，「密集档位要求 4K 分辨率」的门控判定收口在 `server/services/grid_resolution.py`，入队、费用估算、前端预览一律经这两处取档。面向创作者的文档称「分镜板」或「分镜板生视频」，实现和存量界面中也会出现「宫格 / grid」；它是分镜路线内部的装配选项而非生成路线：切割后仍是单张图走 i2v，喂给视频模型的输入契约与逐张分镜完全相同（判据与业界事实见 `docs/adr/0055`、`docs/research/storyboard-to-video-industry-survey.md`）。
