...
制作状态的跨集聚合投影，供项目列表与剧集卡使用：项目所在阶段、可用资产计数、每集可用产物计数与脚本状态。产物判定有两种口径：`verified` 逐件做产物时效比对，供单个项目的详情与剧集卡；`registered` 只看清单登记与文件在场、不读产物字节、stale 恒为 0，供项目列表。两者都以清单为准，不扫目录。
