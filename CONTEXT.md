...
_Avoid_: 把它当成一个自定义 provider（`custom-{id}`）——agent 凭证不进 `ENDPOINT_REGISTRY`、不参与媒体生成；自定义 provider 也不喂给 Agent SDK。
...
**supported_durations**：
某视频模型允许的离散时长集合（秒），是该模型时长的单一真相源；连续区间也整数全展开为离散集（第一方模型恒非空）。剧本 prompt、前端选择器、视频请求体三处同源消费（见 `docs/adr/0018`）。
_Avoid_: `VALID_DURATIONS` / 全局时长白名单（已删的硬编码 `[4,6,8]`，与 per-model 概念相反）；把它当各家「官方时长能力表」（自定义供应商侧只是启发式预填、需用户 review）。
...
分辨率作为**纯清晰度**且 SDK 非必传时，未配置即解析为 None——含义是「调 SDK 时根本不带这个参数」、走 SDK 自身默认，而非我方填兜底默认值；`DEFAULT_VIDEO_RESOLUTION` 等我方默认表已删（见 `docs/adr/0019`）。
_Avoid_: 把 None 当「用某个默认分辨率」而擅自填值。注意当尺寸须**承载比例**时不适用——那走 `aspect_size` 永远算并下传（见 `docs/adr/0011` 与「尺寸」「分辨率」条）。
...
**设计图（sheet）**：
AI 生成的角色/场景/道具定型图（`character_sheet` / `scene_sheet` / `prop_sheet`），是资产生成阶段的**产出**，随后被当作 reference image 喂进下游分镜/宫格/参考生视频以锚定一致性。
_Avoid_: 与「参考图（reference image，喂给生成的条件输入）」混为一谈——方向相反：sheet 是产出后再被引用，参考图是输入；也别与 character 的用户上传 `reference_image` 字段混（那是用户上传的参考文件，非 AI 定型图）。
...
**风格模版（style template）**：
预置的整段画风 prompt 文本（真人 / 动画两类，按 id 选一）。选定时把展开后的 prompt 写入 project.json 的 `style` 字段（供注入用的快照），同时保留 `style_template_id`（可在 PATCH / 读时迁移被重新解析）；registry 改动不主动回灌老项目（见 `docs/adr/0023`）。
_Avoid_: 把 style 理解为短标签（旧值 Photographic/Anime/3D 已废，仅作 legacy 别名懒迁移）；与风格参考图（`style_image`，用户上传的画风参考）叠加——二者互斥，写一个清另一个。
...
_Avoid_: 与 ManagedSession（会话内存状态容器）混为一谈——actor 是执行通道、ManagedSession 是状态；直连 `client.disconnect()` / consumer_task 是已被替代的旧模式。
...
_Avoid_: 用「.claude」「CLAUDE.md」笼统指代——开发态 `.claude/` 与 agent profile 是两套；也别叫 agent config（与 Anthropic 凭证的 agent_config 路由撞名）。
...
Agent 工具调用外围的内核级隔离层（macOS Seatbelt / Linux bwrap），约束**沙箱内所有子进程**（Bash 及其派生进程）的文件读写与网络；SDK 内置 Read/Write/Edit/Glob/Grep 跑在主进程、不经沙箱，靠应用层 PreToolUse hook 拦（见 `docs/adr/0025`、`docs/adr/0026`）。
