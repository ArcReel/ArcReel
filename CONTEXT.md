...
占用台账是 **worker 内存状态**，与 DB 中的 `status='running'` 必须配对维护——cancel 触发时 worker 经 `find_by_task` 找到 asyncio.Task 后 `cancel()`，finally 收尾时 `release` 并把 DB 从 `cancelling` 转 `cancelled`（见 `docs/adr/0006`）。TTS 落地后只需在 CapacityTable 加 audio lane 上限，SlotTable 自然按 `(provider, "audio")` 记账（见 `docs/adr/0010`）。
