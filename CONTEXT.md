...
project.json 顶层字段 `source_fingerprints`（`lib/episode_ledger.SOURCE_FINGERPRINTS_KEY`）：候选源文件（`source/` 直下一级的 .txt/.md，排除派生集文件与下划线前缀文件）相对路径 → 归一化文本（`normalize_source_text` 输出）的 sha256。plan 每次提交对全部候选源文件重新计算并整体覆盖写入（非增量合并），检测「规划中途源文件被外部替换」——账本坐标一旦绑定某段原文，原文再变就意味着坐标失真。比对只针对「已记录」的文件：未记录（存量项目、或新发现源文件首次 plan）不参与比对、不阻塞规划；已记录文件当前指纹不同、或该文件从候选源文件中消失，均判不一致，唯一出路是全量重置（部分重置的前置校验复用同一套比对函数）。plan 与部分重置均锁外预检查一次（快速失败，plan 因此不浪费一次模型调用）、锁内提交闭包内复核一次。plan 的锁内复核之外另有一道基线比对：本次调用实际读入的源文（入口快照 + 中途新发现的文件）在模型调用期间被改动即拒绝提交，这道比对不看是否已记录，首次规划途中换源文同样被拦。全量重置清空整本账本，指纹字段随之清除；部分重置保留段已验证指纹有效，字段不变，留给下一次 plan 提交自然刷新覆盖。源文已耗尽、plan 无内容可规划而早退时不进提交闭包，字段缺失则单独补记一次，避免游标已到底的存量项目永远拿不到基线。
...
`lib/episode_reset.reset_episode_planning` + 同名 SDK 工具，是用户对已规划内容的调整入口——把账本退回未规划状态的逃生口（见 `docs/adr/0032`）。`from_episode=1` 全量重置：零前置校验，任何损坏账本状态都能执行成功，`episodes` 清空、`planning_cursor` 置 null、源文指纹清除。`from_episode>1` 部分重置：保留第 1..from_episode-1 集，前置校验账本形状干净、`from_episode` 为既有集号且保留段集号连续无缺口、退回点（第 from_episode-1 集）坐标结构完整、全部已记录源文指纹一致、保留段坐标落在当前源文界内且首尾相接（跨源文件时前一文件须已耗尽），任一不满足即拒绝执行并指路全量重置。校验只覆盖保留段——重置范围内的条目无论有无坐标都直接清除。两种模式对波及已消费集（已有 step1/剧本/媒体产物）均先返回受影响清单待显式确认（`confirm_consumed=true`）才执行；下游产物一律不删除；重置范围内可由账本重造的派生集文件删除，无原文范围记录的改名留底（避免被后续规划误当孤儿文件重新认领）。重置完成后带调整后的 `instructions` 重新分批调用 plan 即完成调整，若新集号与重置前的已消费范围重叠由 plan 侧的磁盘产物探测自动标 stale。
