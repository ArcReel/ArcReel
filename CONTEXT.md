...
描述一个调用端点协议形态的声明式 JSON，是该端点协议的唯一真相，也是市场条目的载荷；两份定义是否算同一份，看 `meta` 中的作者与名称。
...
按产出媒体共享的排除项片段，图像与视频各一份，以提示词正文里的 `Avoid:` 行渲染，不是供应商参数；生成环节只能追加专属排除项，不得另起措辞。
...
**条目时效（entry currency）**：
按条目内容指纹比对正式剧本与当前脚本规划得出的「哪些条目的内容已经变了」；不含任何准入判定，待修复草稿在场、时长档位或发声准入不满足时照样给出答案，工作流状态与内容确认状态同一口径。
_Avoid_: 与转换预演混用、把「能否转换」的准入结论当作时效。

**转换预演（conversion preview）**：
机械转换的只读预演：过与生成同一组准入断言后回答「现在转换会新增 / 采用 / 移出哪些条目」，任一准入不满足即整体拒绝；只服务「转为正式脚本」这一动作。
_Avoid_: 用它驱动时间线的失效提示、把它当作时效判定。
