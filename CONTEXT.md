...
**补录（backfill）**：
项目 schema 迁移到产物清单时，把项目内既有正式产物按迁移那一刻的内容写入清单并直接认定为 current 的一次性动作；启动期扫描与归档导入共用同一入口，不持久化「来历不明 / legacy」标记，迁移后所有时效判断只走同一规则。补录失败是该项目的阻断性问题——原因原样透出、修复数据后重试，不是切回旧口径的理由。权威索引来源与 v7→v8 激活协议见 `docs/adr/0062`。
