...
**cancelling（取消中）**：
中间状态，表示 cancel 信号已发出但 worker 内 asyncio task 尚未走完 finally 收尾。cancel API 把 DB 从 `running` 改成 `cancelling` 后立即返回；worker finally 在 mark 终态时只能从 `cancelling` 转 `cancelled`（不再走 succeeded/failed 分支）。这是状态机里唯一会被 worker 之外的代码主动写入的状态。
...
**ProviderPool**：
worker 内承载 slot 的数据结构（`lib/generation_worker.py:ProviderPool`），字段 `image_max` / `video_max` + 两个 `inflight: dict[task_id, asyncio.Task]`。inflight 字典是**worker 内存状态**，与 DB 中的 `status='running'` **可能不一致**——比如 cancel 把 DB 改了但 asyncio.Task 没被中断，inflight 仍占着名额（这是已知设计缺陷）。
