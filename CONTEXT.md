...
存量广告参考结构只在项目 schema 迁移时读取：已有 `reference_units` 保留顺序、边界、原样 `unit_id` 与 `generated_assets`，缺失或 null 时按旧 shot 顺序一镜头一单元；成员全悬空时保留身份与历史，迁成 `needs_replan=true`、`shots=[]`、`duration_seconds=0` 的问题壳。该空壳是唯一允许空 shots / 零时长的 unit，运行时阻止生成；历史 `generated_assets.source_signature` 原样留存为惰性键，业务代码不再读取、比较或新增。
