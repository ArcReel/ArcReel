...
**使用记录（usage record）**：
以「一次调用」为行的活动视图，覆盖排队中、生成中与已结束（成功 / 失败 / 取消）的调用；全局设置页承载可筛选的全量视图，项目顶栏悬浮层承载当前项目的精简视图。
_Avoid_: 用量统计、费用明细、任务雷达；面向用户文案里出现账本、落账等记账术语。
...
**引用语法（writing_syntax）**：
创作者在视频单元正文中直接书写的三种记号：`@[名称]` 引用已登记的商品/角色/场景/道具资产（`@[角色/衍生]` 引用该角色的某个衍生），`@[名称]{台词}` 引用资产并附带角色台词（说话人位写衍生时只绑该角色的声音），裸 `{台词}` 标注画外音（不引用任何资产）。
...
**回收站（trash）**：
项目内承接被正式脚本移除的分镜名下全部文件的受控暂存区；分镜从正式脚本移除时，它名下的分镜图、视频、旁白音频与尾帧图连同版本快照一并进入回收站，清单登记同时撤销；由它们派生的字幕与演示物直接删除。回收站内容可下载、单件删除或清空，不随归档导出，也不提供恢复。
_Avoid_: 软删除、把回收站里的东西仍称作产物、把它当作已删除文件的审计日志。

**回收件（trash item）**：
回收站里的一件：一条分镜的一种文件种类（分镜图 / 视频 / 旁白音频 / 尾帧图）连同它的全部版本快照与缩略图附件，以原分镜身份、种类与进入时间识别；它已不是产物，不在清单里，没有产物时效。
_Avoid_: 按物理文件计件、把回收件当 stale 产物。
