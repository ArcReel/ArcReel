...
某次任务入队时冻结的「provider + model」快照（自定义供应商另含 endpoint——它决定协议，provider/model 不变也可能换 backend，故一并冻结），执行与中断续跑都按它进行，不随此后的配置变化漂移。冻结动作称**钉住（pin）**。与「执行模型」相对：执行模型是按当前配置解析求值的结果、随配置变，执行身份是把彼时的执行模型冻结后的快照、此后不变。参考路线入队按 unit 声明近似钉住、提交前按实际参考图把身份改写为实际执行的那一个，续跑因此始终跟随实际执行过的 backend；钉住的身份续跑前只校验存在性与 endpoint 未变，不重跑能力闸（见 `docs/adr/0054`）。图片任务不钉住（任务周期短，配置漂移窗口小）；视频任务的钉住本身是 best-effort——入队时派生执行模型失败会静默留空，此时没有身份可钉，任务退回按当前配置逐次求执行模型（`lib/generation_queue.py::_derive_execution_model_for_enqueue`）。
_Avoid_: 用「执行模型」指代已冻结的身份（一个随配置变、一个不变）；把钉住当能力闸（闸在那个身份被钉住之前已过，钉住只锁身份；入队钉的身份闸在入队时过，参考路线降级为 i2v 的改写身份闸在执行期改写前才过）；换身份续跑（等于拿另一个 backend 轮原身份的 `provider_job_id`）；把「视频任务会钉住」当无条件保证——派生失败时不钉。
