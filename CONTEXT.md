...
描述一个调用端点协议形态的声明式 JSON，是该端点协议的唯一真相；导出、导入与作为市场条目载荷时都是这份 JSON 原样，`meta` 中的作者与名称决定两份定义是否算同一份。
...
**市场源（market source）**：
一个以索引文件列出其市场条目的 GitHub 仓库（或等价的直链地址），在 ArcReel 内登记为一条可启用、可排序的记录；官方市场源随版预置、可禁用不可删除，其余是用户自行添加、自担信任的第三方市场源。
...
**安装（install）**：
把一个市场条目的端点定义落为本地自定义调用端点，并记下它来自哪个市场源的哪个条目；此后该端点跟随市场条目的版本、可原地编辑，删除端点即卸载。从本地文件导入不留这层绑定，不算已安装。
