...
**流式预览态（draft）**：
正在流式生成、尚未完成的 assistant 消息在服务端内存中的唯一预览表示，身份即其消息 id；消息完成时被同 id 的日志权威条目**精确替换**。不入日志、不落盘——服务崩溃即丢，与 agent 记忆一致（SDK 同样不记得未完成的消息）。断线重连时随首帧快照携带当前累积态。
_Avoid_: 用内容比对判断 draft 与已提交内容的重复——对应关系只认消息 id；把 draft 做成日志条目的 pending 状态（破坏日志 append-only）。
