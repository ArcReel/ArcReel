...
**产物（artifact）**：
一次生成落到项目里的正式文件——资产设计图、step1 中间稿、正式脚本、分镜图、视频、旁白音频——连同它与生成依据之间的关系。产物是「用户已付费拥有的东西」，不是任务、不是入队快照、不是执行检查点。
...
清单条目相对当前内容的判定：current（与当前内容一致）/ stale（比当前内容旧：直接依赖已变但文件仍在）/ missing（应有而没有）/ blocked（读不出或依据不可用）。时效只追踪正式内容与直接上游产物，不纳入模型、分辨率、声音参数、供应商配置——配置变化由下一次显式生成生效，不把既有产物变 stale。**stale 仍可用**：可查看、照常参与成片与导出、不阻断工作流，也不是自动重生指令。四态划定与 basis 的界线见 `docs/adr/0062`。
