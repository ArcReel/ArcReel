...
**关键分镜（reference keyframe）**：参考生视频 Step 2 从文稿核心场景切换中定义的静态第一帧预览。以 `@[关键分镜 E1U01K01]` tag 插入 Video Unit 正文，正文位置决定 H3 参考图顺序；每个 Video Unit 最多 5 个，超出时继续拆分 Video Unit。关键分镜内容与数量由用户验收，测试不固化示例语义。

**请求级图片模型（request-scoped image model）**：一次图片生成、重生成或编辑请求携带的 `image_provider` + `image_model` 完整二元组。只影响该任务，不修改项目默认；省略时使用项目默认图片模型。
...
为项目导出签发的短时效凭证，只对导出端点有效（见 `docs/adr/0059`）。
