...
对已激活项目，产物清单是登记每件正式产物的身份、位置与生成依据（它直接消费的正式内容与直接上游产物）的单一真相源；产物是否存在、是否与当前内容一致，一律问清单，不扫目录、不看脚本里的路径串。未激活项目仅在读取路径退回历史行为，不构成生成许可，也不是清单口径的例外（见 `docs/adr/0062`）。
_Avoid_: 读者在读取时推断或修补条目——清单只在补录与正式写入两处变化；把清单条目与执行检查点、provider job、任务状态混为一谈。
...
一次 schema 迁移（含补录）没跑完时，落在项目目录里的一条持久裁决，携带原样失败原因与结构化明细（哪一集 / 哪个文件 / 什么违约）。裁决在则该项目「需要修复」：制作状态与制作计划只报这一条阻断项、Web 与智能体的全部生成入口一律拒绝、项目列表标记出来；已有产物照常可读可看。判定只由**真实失败过的一次尝试**写入——schema 落后但还没轮到迁移的项目不算需要修复；项目达到当前 schema 时（无论经重跑还是本就是最新）裁决清除。唯一的解除路径是按明细修好被点名的数据后重跑迁移链。
_Avoid_: 按 schema 版本号推断阻断（会把「还没轮到迁移」的项目一并封死）；把它当一次性通知（它是持续状态，修好前一直在）；用它阻断只读查看。
