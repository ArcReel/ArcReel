...
def plan_episodes_tool(ctx: ToolContext):
    @tool(
        ...
        "用户提供分集附加指令（如按章节对齐切分、指定某处收尾）时经 instructions 传入原文；附加指令原样注入"
        "规划 prompt 的「附加指令」分节（遵循强度由正文表达，需要强约束时在正文写明），并附带已规划"
        "集数、未规划余量、本窗口体量供换算切分节奏。规划按窗口分多批（长篇会多次调用本工具），instructions 不持久化，"
        "规划全部完成前每一批调用都要重复带上同一附加指令。末批即全部源文规划完毕、或再次调用已无新内容"
        ...
        "reset_episode_planning 做全量重置后才能重新规划。",
        {
            ...
            "properties": {
                "instructions": {
                    "type": "string",
                    "description": (
                        "用户分集附加指令原文（可选，如「按章节对齐切分」）；原样注入规划 prompt 的"
                        "「附加指令」分节，遵循强度由正文表达。每批调用都要重复带上，"
