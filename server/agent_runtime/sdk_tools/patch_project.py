...
把智能体对 ``project.json`` 角色/场景/道具/商品的写入收归 ``patch_project``：按 table
...
# 源语言不会落盘),需要给智能体在用户确认后写入的恢复通道,带 zh/en/vi enum 校验防乱填。
...
def _format_upsert_result(table: str, result: dict[str, Any]) -> str:
    """把 upsert_assets 的诊断 dict 渲染为智能体可读文本。
    ...
    name 维度按字母序排序,渲染顺序稳定不依赖智能体入参 dict 序。
    """
    ...
    if noop:
        ...
        # 误导智能体。dropped_fields / dropped_legacy 段会详述被丢的字段,智能体据此修参。
    ...
    if dropped_fields:
        ...
        lines.append(f"⚠️  以下字段不在智能体可编辑范围,已忽略 → {detail}")
