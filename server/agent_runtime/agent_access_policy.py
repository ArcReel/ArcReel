"""智能体访问规则真相源：内核 sandbox settings 编译与应用层 hook 裁决共用同一份规则。
...
class AgentAccessPolicy:
    ...
    projects_root: Path
    # 智能体 profile 根（已 resolve，受调用方 env 解析控制）：
    ...
    # 项目 session 里的智能体通过 Read/Grep 读到全局日志。无论落在 repo 内还是
    ...
    def build_sandbox_settings(self, project_cwd: Path) -> dict[str, Any]:
        ...
        - ``allowUnsandboxedCommands=False``：禁止智能体在 sandbox 失败时
    ...
    def wrap_bash_command_for_env_scrub(self, command: object) -> str | None:
        ...
        继承全部 env，智能体跑 ``env | grep`` 能看到变量名。通过
    ...
    def is_bash_command_whitelisted(cls, command: str) -> bool:
        ...
        白名单匹配在剥引号 + 反斜杠转正斜杠的归一化串上做：容忍 Windows 智能体发出
    ...
    def _normalize_path_for_protected_compare(cls, path: Path | str) -> str:
        ...
        - ``os.path.normcase`` + ``casefold``：normcase 统一 Windows 分隔符
          ...
          ``project.json`` 指向同一物理文件。Linux case-sensitive 卷上智能体
    ...
    def _is_protected_project_json(cls, target: Path, bases: list[Path]) -> bool:
        ...
        for base in bases:
            ...
            # hook 层须保持一致——否则智能体用 Write 写 scripts/foo.bak / .tmp / .md 会污染剧本
            # 目录，破坏项目结构约定（scripts/ 是剧本 .json 专属，drafts/ 才放草稿）。
            # 同时显式覆盖目录路径本身（target == scripts_dir）：智能体把目录名当文件路径 Write 时
    ...
    def _is_protected_formal_step1(cls, target: Path, bases: list[Path]) -> bool:
        ...
        （迁移读改写、Web 端保存、重生成 / 晋升写盘、智能体修改），除智能体外都持
        ``ProjectManager.file_lock`` 的同一把 per-path 锁；智能体的 Write/Edit 跑在沙箱里、
        ...
        只拦正式文件，不拦同目录的 ``.invalid.json`` 草稿：草稿本就是给智能体用文件工具
