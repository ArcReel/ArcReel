...
项目由会话决定，schema 不含 ``project``。SDK 只把 ``content`` 与 ``isError`` 交给模型，
...
def embedded_result(declaration: ToolDeclaration[Any, Any], outcome: ToolOutcome[Any]) -> types.CallToolResult:
