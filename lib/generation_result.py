...
def recorded_artifact_is_present(
    state: GenerationTargetState, *, manifest_active: bool, project_dir: Path | None
) -> bool:
    ...
    ``project_dir`` of ``None`` means the caller has no directory to resolve a
    recorded relative path against, so the path alone decides.
    """
...
def artifact_is_reusable(state: GenerationTargetState, *, manifest_active: bool, project_dir: Path | None) -> bool:
...
def select_generation_targets(
    *,
    ...
    exists rather than trusting the recorded string. Pass ``None`` only when
    the request genuinely has no project directory.
    """
