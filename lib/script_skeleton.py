...
def resolve_script_kind(script: dict[str, Any]) -> str:
    ...
    **只看数据形状**：本解析回答的是取证提问「这份剧本现在长什么样」，服务于编辑 / 查看 /
    导出——这些能力对任何一份磁盘上的剧本都必须成立，包括骨架与项目路线不符的存量失配剧本
    （集级路线覆盖时代留下的混排集）。若改由项目路线单向定夺，失配集通过所有 MCP 编辑工具
...
class SkeletonRouteMismatchError(ValueError):
    ...
    存量失配剧本的唯一出路是重拆重生成：路线创建时锁定，剧本不可就地换族。查看 / 编辑 /
    导出不经本闸门，失配剧本仍可读可改可导出。
    """

    def __init__(self, *, expected: str, actual: str, generation_mode: str | None) -> None:
        ...
        super().__init__(
            ...
            f"（{SKELETON_ITEM_NOUNS[expected]}）骨架，当前剧本是 {actual}"
            f"（{SKELETON_ITEM_NOUNS[actual]}）骨架。{guidance}。该剧本仍可查看、编辑与导出。"
        )
...
def ensure_route_skeleton(script: dict[str, Any], content_mode: str | None, generation_mode: str | None) -> str:
    ...
    生成分派一律按项目路线（``resolve_declared_kind``），剧本自身不携带路线信息。失配剧本
