...
设计原则：
- 不重复 schema 已声明的枚举（shot_type 等）；让 response_schema 直接约束。camera_motion 为自然语言字段，不限制枚举值。
...
# 镜头运动由 camera_motion 字段独立承载（自然语言），action 只写主体与环境的运动。
_ACTION_WRITING_GUIDE = """首帧画面已定格主体、场景与风格，这段文字驱动它动起来：只描述该时长内主体与环境的运动与变化，不复述画面中的静态内容；镜头运动专由 camera_motion 字段承载。按主体动作（肢体 / 手势 / 表情过渡）、物件互动（摩挲信纸、推门带起的气流等）、环境动态（衣摆、尘埃、雨势、光影移动）分层写成连贯叙述句；动词应描述物理可观察动作（伸手 / 转身 / 摩挲 / 投向 / 收紧），避免内心动词。优先低缓、连贯的细微动作，动作量与该镜头时长匹配：5 秒级镜头通常完成一个连贯动作 + 一个细节互动；8 秒级可承载一次动作过渡（如「抬头—对视—开口」）；更长的镜头保持单一低缓的动作主线，随时长递增动作段数（如「起身—走到窗前—驻足」），而非叠加多条并行动作。
