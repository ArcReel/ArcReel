...
设计原则：
- 不重复 schema 已声明的枚举（shot_type / camera_motion / transition_to_next）；
  让 response_schema 直接约束。
...
_SCENE_WRITING_GUIDE = """用一段连贯的中文描述当前画面中真实可见的元素：角色姿态、面部可观察的状态、环境细节、可见的氛围信号（光线、雾、雨等）。聚焦"此刻这一帧"，不要混入过去/未来事件、抽象情绪词或镜头之外的元素。
...
_ACTION_WRITING_GUIDE = """用一段中文描述该时长内主体的连贯动作（肢体动作、手势、表情过渡），可包含必要的环境互动（衣摆、尘埃、推门带起的气流等）。让画面"活"起来，但不要堆叠不可能在单镜头内完成的动作或蒙太奇切换。
...
d. **image_prompt.composition.shot_type**：按情绪强度与画面信息密度选择枚举值（情绪极致瞬间宜近景 / 特写）。
   **lighting**：{_LIGHTING_WRITING_GUIDE}
   **ambiance**：{_AMBIANCE_WRITING_GUIDE}

e. **video_prompt.action**：{_ACTION_WRITING_GUIDE}

f. **video_prompt.camera_motion**：每个片段只选一种；信息量大或情绪静止时优先 Static。
...
j. **transition_to_next**：默认 "cut"；明显时间 / 空间跳跃时可选 "fade" / "dissolve"。
...
c. **image_prompt.composition.shot_type**：按情绪强度与画面信息密度选择枚举值（情绪极致瞬间宜近景 / 特写）。
   **lighting**：{_LIGHTING_WRITING_GUIDE}
   **ambiance**：{_AMBIANCE_WRITING_GUIDE}

d. **video_prompt.action**：{_ACTION_WRITING_GUIDE}

e. **video_prompt.camera_motion**：每个分镜只选一种；信息量大或情绪静止时优先 Static。
...
i. **transition_to_next**：默认 "cut"；明显时间 / 空间跳跃时可选 "fade" / "dissolve"。

# 创作目标

输出可直接驱动 AI 生成的、视觉一致、节奏紧凑的分镜剧本，适合 {_format_aspect_ratio_desc(aspect_ratio)} 呈现。
"""
