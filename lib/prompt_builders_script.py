...
设计原则：
- 不重复 schema 已声明的枚举（shot_type 等）；让 response_schema 直接约束。camera_motion 已废弃，运镜描述融入 video_prompt.action（自然语言）。
...
_ACTION_WRITING_GUIDE = """首帧画面已定格主体、场景与风格，这段文字驱动它动起来：描述该时长内发生的运动与变化，同时用自然语言说明镜头运动（可包含环绕 / 推进 / 拉远 / 摇移等复合运镜），不复述画面中的静态内容。按镜头运动、主体动作（肢体 / 手势 / 表情过渡）、物件互动（摩挲信纸、推门带起的气流等）、环境动态（衣摆、尘埃、雨势、光影移动）分层写成连贯叙述句；动词应描述物理可观察动作（推进 / 环绕 / 转身 / 摩挲 / 投向 / 收紧），避免内心动词。优先低缓、连贯的细微动作，动作量与该镜头时长匹配：5 秒级镜头通常完成一个连贯动作 + 一个细节互动；8 秒级可承载一次动作过渡（如「抬头—对视—开口」）；更长的镜头保持单一低缓的动作主线，随时长递增动作段数（如「起身—走到窗前—驻足」），而非叠加多条并行动作。
   正例：「镜头缓缓环绕林清，然后向前推进；她抬起头，眼角微微收紧，手指无意识地摩挲信纸边缘；窗外雨势渐大，桌面投下的雨痕影子在缓慢移动。」——镜头运动、主体动作、物件互动、环境动态各有一笔。
