...
v9→v10 用一个写错的 key 前缀解析产物清单，`episode-step1` → `episode-script-plan` 的 kind
改名对真实清单恒为空操作（同一步的草稿路径改写不经过 key 解析，已如实落盘）。残留的旧 key
不会被读侧当作「该产物缺失」：`lib.artifact_manifest` 解析清单时遇到不认识的 kind 判整份清单
不可读，该项目每个产物随即变成 `manifest_unreadable` 阻塞，且除清空清单外没有自愈路径。故
补一次修复，把残留旧 kind 就地改名，其余条目一字不动。

修复复用 v9→v10 的同一份纯转换，对照表只认旧 kind：已在新 key 下的项目是空操作，重跑无副作用。
