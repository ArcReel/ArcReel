...
- 否则 ``episode_target_duration`` 非 ``None`` 时按口播语速折算出阅读单位数，语速取自
  :mod:`lib.speech.speech_rate`，与脚本规划口播时长估算共用同一真相源（含项目级覆盖），两个阶段
  不会各按一套语速走；
...
from lib.speech.speech_rate import project_speech_rate_override, speech_rate_units_per_second
...
def resolve_episode_target_volume(
    ...
    *,
    ...
    语言按 :mod:`lib.speech.speech_rate` 的默认回退）。折算口径为「目标秒数 × 语速」并四舍五入；
