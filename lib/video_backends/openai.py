...
class OpenAIVideoBackend:
    ...
    async def generate(self, request: VideoGenerationRequest) -> VideoGenerationResult:
        ...
        logger.info("OpenAI video download complete: %s", request.output_path)
    ...
    async def _create_video(self, **kwargs):
        """Video generation (create_and_poll) with independent retry."""
    ...
    async def _download_content_with_retry(self, video_id: str):
        """Independently retry content download to avoid re-triggering video generation on download failure."""
