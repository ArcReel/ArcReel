MESSAGES = {
    ...
    "video_capability_missing_i2v": "{provider}/{model} không hỗ trợ tạo video từ ảnh phân cảnh làm khung hình đầu; hãy chỉ định mô hình hỗ trợ tính năng này cho mục tạo video từ ảnh trong Cài đặt, hoặc đổi mô hình video mặc định",
    "video_capability_missing_r2v": "{provider}/{model} không hỗ trợ đầu vào ảnh tham chiếu; hãy chỉ định mô hình hỗ trợ ảnh tham chiếu cho mục tạo video theo tham chiếu trong Cài đặt, hoặc đổi mô hình video mặc định",
