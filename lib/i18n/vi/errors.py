MESSAGES = {
    ...
    "voice_sample_video_audio_unavailable": "Mô hình video hiện tại không thể tạo giọng nói để trích xuất. Hãy dùng TTS hoặc tải âm thanh tham chiếu lên",
    "voice_sample_video_duration_unavailable": "Mô hình video hiện tại không có thời lượng hỗ trợ từ 2 đến 10 giây. Hãy dùng TTS hoặc tải âm thanh tham chiếu lên",
    "voice_sample_strategy_invalid": "Không hỗ trợ cách tạo âm thanh tham chiếu này",
    ...
    "reference_keyframe_not_found": "Không tìm thấy khung hình chính '{id}'",
    "reference_keyframe_limit": "Mỗi đơn vị video chỉ được có tối đa {max_count} khung hình chính; hãy tách thành nhiều đơn vị video hơn",
    "reference_keyframe_task_submitted": "Đã gửi tác vụ tạo khung hình đầu tiên cho '{id}'",
    "reference_keyframes_task_submitted": "Đã gửi {count} tác vụ tạo khung hình đầu tiên",
    "reference_storyboard_sheet_task_submitted": "Đã gửi tác vụ tạo Video Unit Storyboard Sheet cho Video Unit '{unit_id}'",
    "reference_storyboard_sheet_confirmed": "Đã xác nhận Video Unit Storyboard Sheet của Video Unit '{unit_id}' và gửi {count} tác vụ khung hình chính",
    "reference_storyboard_sheet_required": "Video Unit '{unit_id}' phải có Video Unit Storyboard Sheet trước khi tiếp tục",
    "reference_keyframe_plan_required": "Video Unit '{unit_id}' phải xác định ít nhất một keyframe trước khi xác nhận Video Unit Storyboard Sheet",
    "reference_keyframe_images_required": "Video Unit '{unit_id}' vẫn thiếu ảnh đã tạo cho các keyframe: {keyframe_ids}",
    "reference_storyboard_sheet_confirmation_required": "Video Unit Storyboard Sheet của Video Unit '{unit_id}' chưa được xác nhận",
    "reference_storyboard_sheet_input_changed": "Video Unit '{unit_id}' đã thay đổi trong khi tạo Video Unit Storyboard Sheet; hãy tạo lại",
    "reference_storyboard_sheet_changed": "Video Unit Storyboard Sheet của Video Unit '{unit_id}' đã thay đổi; hãy tải lại và xác nhận lại",
    "reference_storyboard_sheet_reference_limit": "Xác nhận Video Unit Storyboard Sheet của Video Unit '{unit_id}' sẽ dùng {count} ảnh tham chiếu, vượt giới hạn {max_count}; hãy tách Unit hoặc giảm tham chiếu",
    "reference_storyboard_sheet_keyframes_enqueue_failed": "Video Unit Storyboard Sheet đã được xác nhận nhưng {failed_count} tác vụ khung hình chính của Video Unit '{unit_id}' không thể gửi",
    ...
    "image_edit_script_file_required": "Cần cung cấp script_file khi chỉnh sửa ảnh phân cảnh hoặc khung hình chính",
