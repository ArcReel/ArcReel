MESSAGES = {
    ...
    "asset_original_missing": "声明的原图读不到：{missing_text}；请重新上传原图，或清除原图字段",
    "generation_input_multiple_gaps": "生成输入有多处缺口：{details}",
    ...
    "grid_task_submitted_with_unsplit": "已提交 {count} 个多宫格分镜生成任务；另有 {unsplit} 张联合图尚未切分落格，请审阅后切分",
    "grid_unsplit_awaiting_split": "{unsplit} 张联合图已就绪、尚未切分落格，请审阅后切分；没有需要新生成的多宫格分镜",
    "grid_nothing_to_generate": "所有分组的分镜图都已就绪，没有需要生成的多宫格分镜",
    "grid_script_route_mismatch": "剧本结构与项目的生成方式不匹配，无法生成多宫格分镜",
