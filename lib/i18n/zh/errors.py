MESSAGES = {
    ...
    "video_capability_missing_i2v": "{provider}/{model} 不支持以分镜图为首帧生成视频；请在设置中为「图生视频」单独指定支持该用途的模型，或更换默认视频模型",
    "video_capability_missing_r2v": "{provider}/{model} 不支持参考图输入；请在设置中为「参考生视频」单独指定支持参考图的模型，或更换默认视频模型",
