MESSAGES = {
    ...
    "grid_task_submitted_with_unsplit": "Submitted {count} multi-grid storyboard generation tasks; {unsplit} combined images are not split yet, review them and split into cells",
    "grid_unsplit_awaiting_split": "{unsplit} combined images are ready but not split yet; review them and split into cells. No multi-grid storyboard needs generating",
    "grid_nothing_to_generate": "Every group already has its storyboard images; no multi-grid storyboard needs generating",
    "grid_script_route_mismatch": "The script structure does not match the project's generation mode, so no multi-grid storyboard can be generated",
