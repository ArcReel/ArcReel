...
- **keyframe_plan（不可为空）**：每个 unit 至少有一个场景，因此必须按出现顺序列出 1–5 个关键场景首帧规划。
  第一项是 unit 开场核心场景在视频 **00:00.000** 的静态第一帧；后续项是每次明显场景建立或切换完成后、该段动作
  尚未推进时的第一帧。每项写清景别 / 角度、构图、主体位置、环境与光线、动作刚要开始或刚开始的静态姿态，
  不写运镜过程、连续动作、动作结果或后续角色反应。若正文动作链是「A 开始 → B 过程 → C 结果」，首帧只能呈现 A，
  绝不能把 B/C 提前塞进首帧。例：正文是「妹妹追赶弟弟，弟弟跑动后摔进桂花堆」，正确首帧是「妹妹在后方刚开始
  追赶、弟弟在前方刚迈步，桂花堆位于前方」；错误首帧是「弟弟摔倒瞬间或脸已埋进桂花堆、妹妹在笑」。
  它不是传统 Storyboard 图片，
...
- 关键帧描述只写这一帧可见的景别、角度、构图、主体位置、环境、光线和起始姿态；不要写运镜过程、连续动作、
  「随后/最终/已经完成」的结果或未来反应。若正文是「妹妹追弟弟，弟弟跑动后摔进桂花堆」，首帧只画妹妹刚开始
  追、弟弟刚迈步，不能画弟弟摔倒、脸埋花堆或妹妹已经发笑。
