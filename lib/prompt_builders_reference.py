...
a. **shots**：{duration_guide_line}
    - `text`：这段文字将直接驱动该镜头的视频生成（语言遵循上方"输出语言"约束）：描述镜头时长内发生的画面与动作，按主体动作（肢体 / 手势 / 神态过渡）、物件互动（拆信、推门、握紧道具等）、环境动态（衣摆、风、雨势、光影移动）分层写成连贯叙述句，可穿插镜头运动（推近 / 拉远 / 横移）。动作量与 `duration` 匹配：短镜头完成一个连贯动作 + 一个细节互动，更长的镜头保持单一动作主线、随时长递增动作段数，不叠加多条并行动作。
        - 角色 / 场景 / 道具仅用 `@[名称]` 引用——外貌、服装、场景陈设等静态外观由参考图承担，**不要**在文本里描写；省下的笔墨用来写足动作与动态。
        - 动词应描述物理可观察动作（伸手 / 转身 / 摩挲 / 投向 / 收紧），避免「陷入 / 回忆 / 意识到 / 决定」等内心动词。
        - 正例：「@[角色A] 在 @[场景A] 中缓步走向窗前，抬手推开木窗，衣摆随穿堂风轻扬；随后低头凝视手中的 @[道具A]，指尖缓缓收紧，镜头自侧后方缓慢推近。」——动作链、物件互动、环境动态、运镜各有一笔。
        - 反例（过短）：「@[角色A] 站在 @[场景A] 里。」——只有静态站位，没有动作过程与环境动态，生成的视频会近乎静止。
