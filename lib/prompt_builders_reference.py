...
- **keyframe_plan（不可为空）**：每个 unit 至少有一个场景，因此必须按出现顺序列出 1–5 个关键场景首帧规划。
  第一项是 unit 开场核心动作或场景 beat 的入口帧；后续项对应 unit 内明显的场景建立或场景切换。入口帧必须是
  动作刚开始、意图与方向已可见的第一个稳定瞬间，不能选择同一 beat 的完成结果、撞击后、摔倒后或事后状态；
  只有结果本身开启新的 beat 时才可成为下一关键帧。每项写清景别 / 角度、构图、主体位置、环境与光线、入口姿态，
  不写运镜过程或连续动作。
...
- 每条关键帧都是对应核心动作或场景 beat 的入口帧：动作刚开始、意图与方向已可见，但结果尚未发生。不得把
  同一 beat 的完成结果、撞击后、摔倒后或事后状态写成入口帧；只有结果本身开启新的 beat 时才可作为下一关键帧。
