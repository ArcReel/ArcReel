    ...
    async with resolver.session() as r:
        ...
        if is_custom_provider(provider_id):
            ...
            async with r._open_session() as (session, _):
                ...
                if provider is None:
                    raise ValueError("The configured custom provider has been deleted. Please re-select the text model in the project settings.")
                name = provider.display_name
                # Verify that model_id still exists and is enabled; otherwise fall back to the default model
                if model_id:
                ...
                if not model_id:
                    ...
                    if default_model:
                        ...
                        raise ValueError(f"Provider '{name}' has no available text models. Please re-select in the project settings.")
