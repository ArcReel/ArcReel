...
const DeclarativeEndpoint = (() => {
  ...
  function normalizeExtractSpec(spec) {
    if (Array.isArray(spec)) return { paths: spec, accept: "string", json_decode: false, then: null, source: "body" };
    return { paths: spec.paths, accept: spec.accept || "string", json_decode: !!spec.json_decode, then: spec.then || null, source: spec.source || "body" };
  }
  ...
  function extract(spec, response) {
    ...
    const root = n.source === "headers" ? lowerKeys(response.headers || {}) : response.body;
    const trace = [];
  ...
  function renderRequest(req, ctx, enumMaps, auth) {
    ...
    const query = {};
    const authQueryKeys = new Set(Object.keys(auth && auth.query || {}));
    for (const [k, v] of Object.entries(req.query || {})) {
      if (authQueryKeys.has(k)) { trace.push(`query ${k}：与 auth 同名，以 auth 为准`); continue; }
  ...
  function encodeInputs(inputsDecl, assets) {
    ...
    for (const [name, decl] of Object.entries(inputsDecl || {})) {
      ...
      const enc = (a) => {
        if (decl.mime_types && !decl.mime_types.includes(a.mime)) throw new RenderError(`inputs.${name}：素材 MIME ${a.mime} 不在白名单 [${decl.mime_types.join(", ")}] 内——不发请求`);
  ...
  const BUILTIN_STATUS_SYNONYMS = {
    ...
    expired: "expired", not_found: "expired", timeout: "failed", timed_out: "failed",
  ...
  // ProviderJobStatus 五个字面量；运行时只区分 继续轮询(queued|running) / succeeded / failed / expired
  const CANONICAL_STATUSES = ["queued", "running", "succeeded", "failed", "expired"];
  function mapStatus(raw, statusMap) {
    ...
    // 五个字面量之外的目标一律折为 failed，不让未知终态漏进轮询循环
  ...
  function validateDefinition(def) {
    ...
    for (const sec of ["submit", "poll"]) {
      ...
      for (const f of ["url", "headers", "query", "body"]) if (r[f] !== undefined) scan(r[f], `${sec}.${f}`, scope);
      if (sec === "poll" && !JSON.stringify([r.url, r.query, r.body]).includes("task_id")) warn("poll", "轮询请求没有引用 {{ task_id }}——确认这是有意的");
      ...
      for (const k of Object.keys(r.query || {})) if (((def.auth || {}).query || {})[k] !== undefined) err(`${sec}.query.${k}`, `与 auth.query 同名——凭证 query 参数只能由 auth 节写入`);
      // 路径子集
    ...
    for (const [k, v] of Object.entries(def.status_map || {})) if (!CANONICAL_STATUSES.includes(v)) err(`status_map.${k}`, `目标档位 ${JSON.stringify(v)} 不在 queued / running / succeeded / failed / expired 内`);
    for (const [k, v] of Object.entries(def.poll && def.poll.extract || {})) { const n = v && !Array.isArray(v) && typeof v === "object" ? v : null; if (n && n.source === "status_code") err(`poll.extract.${k}.source`, "status_code 不是提取来源——HTTP 码用 poll.success_status_codes / retry_status_codes / expired_status_codes 处理"); }
    ...
    if (caps.first_frame && !Object.values(inputs).some((d) => d.source === "start_image")) warn("capabilities.first_frame", "声明支持首帧，但 inputs 没有 start_image 来源——首帧会被静默丢弃");
    if (caps.last_frame && !Object.values(inputs).some((d) => d.source === "end_image")) warn("capabilities.last_frame", "声明支持尾帧，但 inputs 没有 end_image 来源");
  ...
  function reduce(state, action) {
    ...
    switch (action.type) {
      ...
      case "SUBMIT": {
        ...
        const ok = (def.submit.success_status_codes || [200, 201, 202]).includes(action.response.status_code);
        if (!ok) {
      ...
      case "POLL": {
        ...
        // HTTP 层：成功码之下才看 body 状态；retry 码保持上一状态继续轮询；expired 码 ⇒ 任务已过期；其余码 ⇒ 任务失败
        const okCodes = def.poll.success_status_codes || [200];
        const retryCodes = def.poll.retry_status_codes || [429, 500, 502, 503, 504];
        const expiredCodes = def.poll.expired_status_codes || [404];
        if (!okCodes.includes(resp.status_code)) {
          const prev = s.status ? s.status.canonical : "queued";
          if (retryCodes.includes(resp.status_code)) {
            s.poll = { request: req, response: resp, extract: {}, usage: {}, mapped: { raw: `HTTP ${resp.status_code}`, key: null, source: "retry_status_codes", canonical: prev } };
            s.phase = "polling"; say(`… HTTP ${resp.status_code} 在 retry_status_codes 内，保持 ${prev}，${def.poll.interval_seconds || 5}s 后再轮询`); return s;
          }
          if (expiredCodes.includes(resp.status_code)) {
            s.poll = { request: req, response: resp, extract: {}, usage: {}, mapped: { raw: `HTTP ${resp.status_code}`, key: null, source: "expired_status_codes", canonical: "expired" } };
            s.status = s.poll.mapped;
            s.phase = "expired"; s.error = `HTTP ${resp.status_code}：provider 端任务已过期 / 不存在`; say(`⌛ ${s.error}（provider 已不认识该任务：终态，重启接续时按可重试处理）`); return s;
          }
          const er = def.poll.extract.error ? extract(def.poll.extract.error, resp) : { value: null, trace: [] };
          s.poll = { request: req, response: resp, extract: { error: er }, usage: {}, mapped: { raw: `HTTP ${resp.status_code}`, key: null, source: "success_status_codes", canonical: "failed" } };
          s.status = s.poll.mapped;
          s.phase = "failed"; s.error = `HTTP ${resp.status_code}：${er.value || "（错误路径无命中）"}`; say(`✗ 轮询响应码不在 success_status_codes 内 ⇒ 任务失败 ${s.error}`); return s;
        }
        ...
        if (mapped.canonical === "expired") {
          const er = def.poll.extract.error ? extract(def.poll.extract.error, resp) : { value: null, trace: [] };
          s.poll.extract.error = er;
          s.phase = "expired"; s.error = er.value || "provider 端任务已过期 / 不存在"; say(`⌛ 任务已过期：${s.error}（provider 已不认识该任务：终态，重启接续时按可重试处理）`); return s;
        }
        s.phase = "polling"; say(`… ${mapped.canonical}，${def.poll.interval_seconds || 5}s 后再轮询`); return s;
      }
      default: return s;
    }
  }

  return { parsePath, evalPath, extract, normalizeExtractSpec, renderRequest, renderNode, encodeInputs, mapStatus, BUILTIN_STATUS_SYNONYMS, validateDefinition, initialState, reduce, buildContext, PathError, RenderError, DROP };
})();
