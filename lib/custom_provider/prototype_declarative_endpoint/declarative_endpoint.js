...
const DeclarativeEndpoint = (() => {
  ...
  function deriveCapabilities(def) {
    ...
    const inputs = Object.entries(def.inputs || {}).filter(([name]) => used.has(name)).map(([, d]) => d);
    return {
  ...
  return { parsePath, evalPath, extract, normalizeExtractSpec, renderRequest, renderNode, encodeInputs, mapStatus, BUILTIN_STATUS_SYNONYMS, validateDefinition, deriveCapabilities, referencedInputs, initialState, reduce, buildContext, PathError, RenderError, DROP };
})();
