...
const DeclarativeEndpoint = (() => {
  ...
  const BUILTIN_STATUS_SYNONYMS = {
    ...
    expired: "failed", timeout: "failed", timed_out: "failed",
  ...
  function mapStatus(raw, statusMap) {
    ...
    // 四档之外的目标一律折为 failed，不让第五档漏进轮询循环
  ...
  function validateDefinition(def) {
    ...
    for (const [k, v] of Object.entries(def.status_map || {})) if (!CANONICAL_STATUSES.includes(v)) err(`status_map.${k}`, `目标档位 ${JSON.stringify(v)} 不在四档内（expired / timeout 请映射到 failed）`);
    for (const [k, v] of Object.entries(def.poll && def.poll.extract || {})) { const n = v && !Array.isArray(v) && typeof v === "object" ? v : null; if (n && n.source === "status_code") err(`poll.extract.${k}.source`, "status_code 不是提取来源——HTTP 码用 poll.success_status_codes / retry_status_codes 处理"); }
  ...
  function reduce(state, action) {
    ...
    switch (action.type) {
      ...
      case "POLL": {
        ...
        // HTTP 层：成功码之下才看 body 状态；retry 码保持上一状态继续轮询；其余码 ⇒ 任务失败
