...
const DeclarativeEndpoint = (() => {
  ...
  // URL 字符串（模板或渲染结果）里 ? 之后的参数名集合
  function urlQueryParamNames(url) {
    const i = url.indexOf("?");
    if (i < 0) return new Set();
    return new Set(url.slice(i + 1).split("#")[0].split("&").filter(Boolean).map((kv) => decodeURIComponent(kv.split("=")[0])));
  }
  // submit 里实际引用到的 inputs 名（{{ inputs.x }} 或 $each.in）；能力推导与「声明未用」校验都以此为准
  function referencedInputs(submit) {
    const text = JSON.stringify(submit || {});
    const names = new Set();
    for (const m of text.matchAll(/\{\{\s*inputs\.([a-z][a-z0-9_]*)\s*\}\}/g)) names.add(m[1]);
    for (const m of text.matchAll(/"in":"inputs\.([a-z][a-z0-9_]*)"/g)) names.add(m[1]);
    return names;
  }
  function renderRequest(req, ctx, enumMaps, auth) {
    ...
    const urlQueryNames = urlQueryParamNames(url);
    for (const k of Object.keys(query)) if (urlQueryNames.has(k)) throw new RenderError(`URL 自带的 query 参数 ${k} 与 auth.query 同名——凭证 query 只能由 auth 节写入`);
  ...
  // 能力全部显式声明（唯一来源）；这里只补默认值。与 submit 引用的一致性由 validateDefinition 两向检查
  const CAPABILITY_DEFAULTS = {
    first_frame: false, last_frame: false, max_reference_images: 0, reference_audio_mode: "none", max_reference_audio_count: 0, max_reference_audio_total_seconds: null,
  ...
  function resolveCapabilities(def) {
  ...
  function validateDefinition(def) {
    ...
    const used = referencedInputs(def.submit);
    for (const name of Object.keys(inputs)) if (!used.has(name)) err(`inputs.${name}`, "声明了但 submit 里没有引用——未引用的素材不会发给供应商，也不能据此推导能力");
    const authQueryNames = Object.keys((def.auth || {}).query || {});
    for (const sec of ["submit", "poll"]) {
      const u = def[sec] && def[sec].url; if (typeof u !== "string") continue;
      for (const k of urlQueryParamNames(u)) if (authQueryNames.includes(k)) err(`${sec}.url`, `URL 自带的 query 参数 ${k} 与 auth.query 同名——凭证 query 只能由 auth 节写入`);
    }
    ...
    // 能力声明 ⇄ submit 实际引用的素材来源：两向一致，撒谎的两个方向都封住
    const caps = resolveCapabilities(def);
    const usedSources = new Set(Object.entries(inputs).filter(([name]) => used.has(name)).map(([, d]) => d.source));
    const pairs = [
      ["first_frame", "start_image", caps.first_frame === true, "first_frame: true"],
      ["last_frame", "end_image", caps.last_frame === true, "last_frame: true"],
      ["max_reference_images", "reference_images", caps.max_reference_images > 0, "max_reference_images > 0"],
      ["reference_audio_mode", "reference_audio_files", caps.reference_audio_mode !== "none", "reference_audio_mode ≠ none"],
    ];
    for (const [field, source, declared, label] of pairs) {
      if (declared && !usedSources.has(source)) err(`capabilities.${field}`, `声明了 ${label}，但 submit 没有引用任何 ${source} 来源的 input——能力会撒谎`);
      if (!declared && usedSources.has(source)) err(`capabilities.${field}`, `submit 引用了 ${source} 来源的 input，但没有声明 ${label}——素材会发出去而 UI 不开放该能力`);
    }
    return issues;
  }
  ...
  return { parsePath, evalPath, extract, normalizeExtractSpec, renderRequest, renderNode, encodeInputs, mapStatus, BUILTIN_STATUS_SYNONYMS, validateDefinition, resolveCapabilities, CAPABILITY_DEFAULTS, referencedInputs, initialState, reduce, buildContext, PathError, RenderError, DROP };
})();
