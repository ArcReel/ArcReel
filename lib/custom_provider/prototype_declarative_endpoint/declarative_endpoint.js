...
const DeclarativeEndpoint = (() => {
  ...
  function extract(spec, response) {
    ...
    const root = n.source === "body" ? response.body : n.source === "headers" ? lowerKeys(response.headers || {}) : response.status_code;
    const trace = [];
  ...
  function renderRequest(req, ctx, enumMaps, auth) {
    ...
    const headers = {};
    for (const [k, v] of Object.entries({ ...(auth && auth.headers || {}), ...(req.headers || {}) })) {
      const r = renderString(v, ctx, null, trace); if (r !== DROP) headers[k] = stringify(r);
    }
    const query = {};
    for (const [k, v] of Object.entries({ ...(auth && auth.query || {}), ...(req.query || {}) })) {
  ...
  function mapStatus(raw, statusMap) {
    ...
    if (statusMap && Object.prototype.hasOwnProperty.call(statusMap, key)) return { raw, key, source: "status_map", canonical: statusMap[key] };
    if (Object.prototype.hasOwnProperty.call(BUILTIN_STATUS_SYNONYMS, key)) return { raw, key, source: "builtin", canonical: BUILTIN_STATUS_SYNONYMS[key] };
    return { raw, key, source: "fallback", canonical: "running" };
  }
  ...
  function validateDefinition(def) {
    ...
    for (const [k, v] of Object.entries(def.status_map || {})) if (!["queued", "running", "succeeded", "failed"].includes(v)) err(`status_map.${k}`, `目标档位 ${JSON.stringify(v)} 不在四档内（expired 不由声明式产生）`);
