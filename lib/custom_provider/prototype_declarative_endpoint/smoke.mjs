...
const removed = { ...gdef, submit: { ...gdef.submit, query: { a: "b" }, success_status_codes: [200] }, poll: { ...gdef.poll, interval_seconds: 5 }, capabilities: { ...gdef.capabilities, last_frame: true }, meta: { ...gdef.meta, media_type: "video" } };
const removedIssues = D.validateDefinition(removed).filter((i) => i.level === "error").map((i) => i.where).sort();
t("removed fields flagged: " + removedIssues.join(","), removedIssues.join(",") === "capabilities.last_frame,meta.media_type,poll.interval_seconds,submit.query,submit.success_status_codes");
const caps = D.resolveCapabilities(gdef);
t("declared caps + defaults", caps.first_frame === true && caps.last_frame === false && caps.audio_track === "controllable" && caps.max_reference_images === 0);
const unused = { ...gdef, inputs: { ...gdef.inputs, tail: { source: "end_image", encoding: "base64" } } };
t("unreferenced input → error", D.validateDefinition(unused).some((i) => i.level === "error" && i.where === "inputs.tail"));
const lie1 = { ...gdef, capabilities: { ...gdef.capabilities, last_frame: true } };
t("declared last_frame without referenced end_image → error", D.validateDefinition(lie1).some((i) => i.level === "error" && i.where === "capabilities.last_frame"));
const lie2 = { ...gdef, capabilities: { ...gdef.capabilities, first_frame: false } };
t("referenced start_image without first_frame → error", D.validateDefinition(lie2).some((i) => i.level === "error" && i.where === "capabilities.first_frame"));
const lie3 = { ...gdef, capabilities: undefined };
t("capabilities omitted but start_image referenced → error", D.validateDefinition(lie3).some((i) => i.level === "error" && i.where === "capabilities.first_frame"));
