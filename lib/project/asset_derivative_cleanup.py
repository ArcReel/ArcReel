...
单列一个模块是因为它同时需要 :mod:`lib.artifacts.artifact_registration`（清单）与
:mod:`lib.artifacts.version_manager`（历史），而 :mod:`lib.project.asset_derivatives` 被产物规划反向依赖，
不能再引入这两者。
"""
...
from lib.artifacts.artifact_registration import forget_current_resource_artifact
