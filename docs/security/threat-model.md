...
| FastAPI → external providers | Credentials, prompts, media, base URLs, job IDs | Authentication, provider registry, configured endpoints, HTTP timeouts | SSRF, secret forwarding, malformed responses, unexpected cost |
| Provider → ArcReel download/parser pipeline | URLs, response headers, media bytes | HTTP client timeouts, artifact-path handling, downstream format checks | SSRF, memory/disk exhaustion, parser compromise |
