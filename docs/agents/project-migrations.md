...
| 宫格与参考视频的依据只记 `style`，没有风格描述 | schema ≤ 15 | 描述非空时另记扁平 `style_description` | v15→v16 只改写改写前时新的登记，本就过期的保留；参考视频的冻结依据另按新口径重盖章 |
...
- 同一件产物的依据可能同时落在清单条目与版本记录里（参考视频的冻结依据存于记录，成片读模型拿它与现算的重建依据比对）。只改一侧会让两个读模型互相矛盾，故这类步骤两侧都要按同一口径改，且只改「改写前正是时新」的那些（先例：v15→v16 重盖章参考视频依据）。改记录的路径要让重跑收敛：改写前那侧的目标态不能反过来依赖记录当下的形态。
