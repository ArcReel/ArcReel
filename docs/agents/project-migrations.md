...
| 分镜图已生成，但它引用的资产在激活时没有可登记的资产图（资产已删、没有资产图、资产图文件丢失，或描述为空致资产图不登记）；旧执行器遇到这类缺口静默丢图照常出图 | 任意 | 分镜图的生成输入成立（引用全部登记且资产图可用）才登记 | 不登记该分镜图，进迁移报告，读时报 missing；上一分镜图因此未登记时，下一张只是不带它 |
