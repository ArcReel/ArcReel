...
11. **运行时 plugin 的 ENDPOINT_REGISTRY 动态化方案**（静态字典 → 内置+plugin 两层结构；与 9.1 第 3 点同源）
12. **运行时 plugin 的安全模型**（代码级沙箱已证不可行，重心放在来源信任 + 审核机制）
13. **运行时 plugin 的发现与加载机制选型**（entry_points / 目录扫描 / 两者）
14. **运行时 plugin 的契约定义**（元数据声明、凭证注入、i18n、启发式规则）
15. **运行时 plugin 的渐进式落地阶段划分**（最小可用 → 社区分享 → 官方市场）
