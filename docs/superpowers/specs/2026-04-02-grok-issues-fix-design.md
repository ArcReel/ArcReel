...
在 `image_utils.py` 新增压缩函数，上传入口统一调用：

- **格式**：转 JPEG（quality=85）
- **分辨率**：长边不超过 2048px，等比缩放
- **适用范围**：仅用户上传的图片（角色参考图、角色图、线索图、风格参考图），AI 生成的图片暂不处理
...
- `files.py` 上传逻辑硬编码 `.png` 后缀（lines 103-124），需改为 `.jpg`
- `project.json` 中的 `character_sheet`、`reference_image`、`clue_sheet` 引用路径将自然变为 `.jpg`
- 下游代码（`image_to_base64_data_uri`、`_collect_reference_images` 等）通过路径读取文件，不依赖特定后缀，无需改动
- 已存储的旧 PNG 文件无需迁移，仍可正常使用
- `style_reference.png` 路径也需改为 `.jpg`（`files.py` line 513）
...
额外增加 aspect_ratio 支持列表校验作为兜底：Grok 支持 `16:9`, `9:16`, `4:3`, `3:4`，不支持的比例映射到最近值并发出警告日志。
