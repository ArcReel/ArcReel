...
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `lib/text_backends/instructor_support.py` | Create | Instructor 降级函数 |
| `lib/text_backends/ark.py` | Modify | 构造时读取能力，降级路径调用 instructor_support |
| `lib/config/registry.py` | Modify | Fix doubao model capabilities |
| `lib/project_manager.py` | Modify | response_schema 传 Pydantic 类 |
| `pyproject.toml` | Modify | 添加 `instructor>=1.7.0` 依赖 |
| `tests/test_text_backends/test_instructor_support.py` | Create | instructor_support unit tests |
| `tests/test_text_backends/test_ark.py` | Modify | 新增能力判断 + 降级路径测试 |
