...
- **PreToolUse 文件围栏简化为「跨项目隔离」唯一职责**：原扩展名白名单、project_root 共享读放行等分支全部删除
...
| **L4a** Permission rules (settings.json) | 文件读写边界（同时管 SDK 工具与翻译给 sandbox） | `permissions.deny` 扩展敏感文件 + 写代码文件 deny；`allow` 整段清空 |
| **L4b** PreToolUse hook | SDK 内置工具的动态跨项目隔离 | hook 精简：只拦 `projects/<other>/` 跨项目；保留 JSON 校验 |
| **L4c** `allowed_tools` (代码) | 工具白名单集中声明 | 加入 `Bash`/`BashOutput`/`KillBash`，其余保留 |

设计要点：
- L3 是新增核心防线（让 Bash 自由），L4a 同时被 sandbox 翻译给底层 profile
- SDK 文档（`claude_agent_sdk/types.py:875-882`）明确：**sandbox 启用后，文件读限制走 `Read(<path>)` deny rules、写限制走 `Edit(...)` allow/deny rules、网络限制走 `WebFetch(...)` rules**。这是 sandbox 的统一控制面 —— 详见 PoC #2 验证
- L4b 与 L4a 职责不重叠：deny rules 静态（路径模式），hook 动态（运行时 cwd 比对）
...
| `_is_path_allowed()` | 精简：在 `project_root/projects/` 下但不在当前 cwd 内 → 拒；其余通过；SDK tool-results / `/tmp/claude-*/tasks` 保留例外 | 跨项目隔离唯一职责 |
| 删 `_WRITE_TOOLS` / `_WRITABLE_EXTENSIONS` 常量 | 扩展名白名单交给 deny rules + prompt 约束 | 简化 |
...
```jsonc
{
  "permissions": {
    "deny": [
      // —— 写代码文件 deny（替代旧 hook 扩展名白名单）——
      "Edit(//app/**/*.py)", "Edit(//app/**/*.js)", "Edit(//app/**/*.ts)",
      "Edit(//app/**/*.tsx)", "Edit(//app/**/*.sh)", "Edit(//app/**/*.yaml)",
      "Edit(//app/**/*.yml)", "Edit(//app/**/*.toml)",
      "Write(//app/**/*.py)", "Write(//app/**/*.js)", "Write(//app/**/*.ts)",
      "Write(//app/**/*.tsx)", "Write(//app/**/*.sh)", "Write(//app/**/*.yaml)",
      "Write(//app/**/*.yml)", "Write(//app/**/*.toml)",

      // —— 仓库代码 / 文档 / 运行时配置目录写 deny ——
      "Edit(//app/docs/**)", "Edit(//app/lib/**)",
      "Edit(//app/agent_runtime_profile/**)", "Edit(//app/scripts/**)",
      "Edit(//app/alembic/**)",
      "Write(//app/docs/**)", "Write(//app/lib/**)",
      "Write(//app/agent_runtime_profile/**)", "Write(//app/scripts/**)",
      "Write(//app/alembic/**)",

      // —— 敏感文件 deny（读 + 写都拒；sandbox 启用后翻译给 Bash 子进程）——
    ...
    // 注意：原 "allow" 整段已删除。Bash / BashOutput / KillBash / Read / Grep / Glob
...
| `tests/agent_runtime/test_path_isolation_hook.py` | 新版精简 `_is_path_allowed`：(1) cwd 内通过；(2) `projects/<other>/...` 拒；(3) SDK tool-results / `/tmp/claude-*/tasks` 例外 |
...
5. agent 跑 `Write({"file_path": "/app/projects/<self>/test.py", "content": "..."})` → deny rule 拒
...
- 现有 hook 测试涉及 `_WRITABLE_EXTENSIONS` 的需删
