  ...
  return (
    <ul className="grid gap-2.5">
      {credentials.map((c) => (
        <li
          key={c.id}
          className="flex items-center gap-3 rounded-[10px] border border-hairline px-3 py-3"
          style={CARD_STYLE}
        >
...
export function AddCredentialModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[12px] border border-hairline bg-bg p-5"
        style={CARD_STYLE}
      >
        {/* Header */}
        <div className="mb-4 flex items-start justify-between">
          <h3 className="text-[15px] font-medium text-text">{t("add_credential")}</h3>
...
function PresetChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`relative inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px] transition ${
        selected
    ...
    >
      ...
      {recommended && (
        <Star
          className="absolute -right-0.5 -top-1 h-3 w-3 fill-amber-400 text-amber-400"
...
export function TestResultPanel({ result, onApplyFix }: Props) {
  const { t } = useTranslation("dashboard");
  const { overall, messages_probe, discovery_probe, diagnosis, suggestion, derived_messages_root, derived_discovery_root } = result;
  ...
  return (
    <div className={`mt-3 rounded-[10px] border p-3 ${tone}`}>
      ...
      {suggestion?.kind === "replace_base_url" && suggestion.suggested_value && onApplyFix && (
        <div className="mt-2 flex items-center justify-between gap-2 rounded-[8px] border border-hairline-soft bg-bg-grad-a/35 px-2.5 py-2">
          <span className="truncate font-mono text-[10.5px] text-text-3">
            {t("apply_fix_hint")}
            <span className="ml-1 text-text-2">{suggestion.suggested_value}</span>
          </span>
          <button
            type="button"
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 font-mono text-[10.5px] text-text-4">
        ...
        <div>
          ...
          <div className="text-text-4">
            GET ·{" "}
            {discovery_probe ? `${discovery_probe.status_code ?? "—"} · ${discovery_probe.latency_ms ?? "—"}ms` : "—"}
          </div>
        </div>
      </div>
...
const handleApplyFix = async (suggestedUrl: string) => {
  // 找到刚才被测的凭证 (testResult 当时来自哪个 id 由 busyCredId 跟踪不稳定，
  // 改用：取当前显示的 testResult 上下文 — 简化实现：要求用户在 modal 里手动覆盖 base_url)
  // 这里只更新 active credential 的 base_url
  const active = credentials.find((c) => c.is_active);
  if (!active) return;
  await API.updateAgentCredential(active.id, { base_url: suggestedUrl });
  await loadCreds();
  setTestResult(null);
  useAppStore.getState().pushToast(t("agent_config_saved"), "success");
};
