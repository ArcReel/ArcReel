    ...
    async def send_new_session(
        ...
        *,
        ...
        def _on_actor_message(msg: dict[str, Any]) -> None:
            ...
            if managed is None:
                return
            # 状态机 + buffer + broadcast
            managed._on_actor_message(msg)
            # SessionManager 侧的特殊消息处理（pending_questions 登记、
            # sdk_session_id 捕获、final result handling 等）保留
            self._handle_special_message(managed, msg)
...
- [ ] **Step 11.4: 修复 session_manager_more 里被 send_new_session 改动影响的测试**
...
```bash
git add server/agent_runtime/session_manager.py tests/test_session_manager_more.py
git commit -m "refactor(session-actor): send_new_session 走 actor 生命周期 (#159)"
```
...
- [ ] **Step 18.2: 删除 _consume_messages**

删除 `_consume_messages` 方法体（约 1049-1076 行）。`_handle_special_message`、`_finalize_turn`、`_mark_session_terminal` 视情况保留（它们处理 `ask_user_question` / final result；可能被 `_handle_special_message` 用）。
