...
**策略**：上传图片 > 2MB 时才压缩为 JPEG（`.jpg`），≤ 2MB 保持原有 PNG 转换逻辑（`.png`）。
...
3. 替换图片处理逻辑（原 lines 132-138），根据大小选择压缩或 PNG 转换，并动态决定后缀：

```python
        # 保存文件（大图压缩为 JPEG，小图转 PNG）
...
4. 将文件名后缀从硬编码 `.png` 改为使用动态 `ext` 变量。把文件名决定逻辑移到图片处理之后：
...
```python
        ...
        if upload_type == "source":
            ...
            if upload_type == "storyboard":
                filename = f"scene_{name}{ext}" if name else f"{Path(file.filename).stem}{ext}"
            elif name:
                filename = f"{name}{ext}"
            else:
...
注意：以上是逻辑重组的方向，实际实现时需基于完整的 `files.py` 代码适配，保持与 `source` 类型的分支结构一致。核心要点是 **先读取内容判断大小 → 决定 ext → 再拼文件名**。
...
```python
        ...
        try:
            if len(content) > _COMPRESS_THRESHOLD:
                processed = compress_image_bytes(content)
                style_filename = "style_reference.jpg"
            else:
                processed = convert_image_bytes_to_png(content)
