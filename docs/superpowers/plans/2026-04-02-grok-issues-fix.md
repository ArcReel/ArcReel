...
1. 将 import `convert_image_bytes_to_png` 改为同时导入 `compress_image_bytes`：

```python
from lib.image_utils import compress_image_bytes, convert_image_bytes_to_png
```

2. 将所有上传类型的后缀从 `.png` 改为 `.jpg`（lines 103-124）：
...
```python
            ...
            if name:
                filename = f"scene_{name}.jpg"
            else:
                filename = f"{Path(file.filename).stem}.jpg"
```

3. 将图片转换从 `convert_image_bytes_to_png` 改为 `compress_image_bytes`（line 136）：
...
在 `server/routers/files.py` 的 `upload_style_image` 函数中（lines 506-541）：

1. 将 `convert_image_bytes_to_png` 改为 `compress_image_bytes`：

```python
        try:
            compressed_content = compress_image_bytes(content)
        except ValueError:
            raise HTTPException(status_code=400, detail="无效的图片文件，无法解析")
```

2. 将所有 `style_reference.png` 改为 `style_reference.jpg`：

```python
        output_path = project_dir / "style_reference.jpg"
        with open(output_path, "wb") as f:
            f.write(compressed_content)
```

```python
        project_data["style_image"] = "style_reference.jpg"
```
...
```python
        return {
            ...
            "url": f"/api/v1/files/{project_name}/style_reference.jpg",
        }
```

3. 同样更新 `delete_style_image` 函数中的路径（line 561）：

```python
        image_path = project_dir / "style_reference.jpg"
```

- [ ] **Step 3: 更新 data_validator 和 project_archive 中的硬编码路径**

`lib/data_validator.py` line 50 — 在 `ALLOWED_ROOT_ENTRIES` 中添加 `.jpg` 变体：
...
`server/services/project_archive.py` line 492 — `_repair_path_to_canonical` 调用中的 `canonical_rel` 参数改为 `"style_reference.jpg"`：
...
```python
        if self._repair_path_to_canonical(
            ...
            canonical_rel="style_reference.jpg",
...
Expected: 全部 PASS（部分测试可能需要适配 `.jpg` 后缀）

- [ ] **Step 5: 提交**

```bash
git add server/routers/files.py lib/data_validator.py server/services/project_archive.py
git commit -m "feat: 上传图片改用 JPEG 压缩存储，长边限制 2048px"
```
