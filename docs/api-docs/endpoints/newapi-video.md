...
- 任务状态与回包形状：状态串随底层厂商透传，过共享归一 `lib/backends/video_backend_contract.py::normalize_provider_status`；状态、视频地址与 metadata 按定义 `poll.extract` 的路径表并集探测，兼容扁平与 `data` 包装两种回包
