...
- ③ 是 POSIX 机制（依赖 `env`/`sh`），仅在沙箱可用（macOS/Linux）时执行包装；hook 只改写命令、不下权限决策，放行由权限链后续步骤（allowed_tools）完成。Windows 原生回退下 hook 跳过包装，原始命令交由 `can_use_tool` 的 Bash 白名单（token 边界匹配 + shell metachar 拦截）裁决——①② 两层不受平台影响。
