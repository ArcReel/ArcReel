...
ComfyUI 本体零鉴权，用户加的反向代理鉴权方式各异（header 或 query）。凭据头模板放在端点定义的 `auth` 节，与声明式端点同一 schema，`{{api_key}}` 只许出现在此节，`api_key` 留空时整节不渲染。供应商行不为此加列，`docs/adr/0008` 的单字段凭证模型原样守住。已知代价：鉴权方式按部署而异，同一 workflow 挂到两台鉴权不同的机器需复制一份定义；接受为边缘场景。
...
ComfyUI 是用户自己的显卡，没有供应商账单。不发明 GPU 计价：费用沿用模型行现有单价字段与现有公式，单价为空即 0；`duration_ms` 即执行时长，供企业分摊场景手填单价后按时长估算。记账层零改动。

## 取消与超时叫停远端，在 backend 内捕获而不扩协议

ArcReel 放弃一次生成时（用户取消或轮询超时）best-effort 通知 ComfyUI 取消该任务，释放显卡；这是全仓库唯一通知供应商的取消——商业 API 的任务放弃后只是不再轮询，供应商侧照跑照计费，通知没有意义，而 ComfyUI 侧不叫停就白占用户显卡。实现不扩 `VideoBackend` / `ImageBackend` 协议：ComfyUI backend 在 `generate()` 内捕获 `CancelledError`，best-effort 远端取消后重新抛出；远端失败只记日志，本地状态机（`docs/adr/0006`）不受影响。超时路径同一机制。代价是显卡慢的用户可能在超时上限丢掉快跑完的结果，由可配置的轮询超时缓解。
...
- `static_media_type` 对 `ce-` 键恒判 video 的推断退役，改读镜像列 / 已解析的 `EndpointSpec.media_type`。
- 端点测试对 ComfyUI 端点只有预览请求与测试连接；验证响应不提供——产物提取是固定代码，无用户可配的取值路径可验。
- 首期只轮询 `/history`，不接 WebSocket；不显示 ComfyUI 侧排队位次，不报假进度。
- 后续若要把 workflow + 节点绑定作为新的市场条目类型、或接入托管型 ComfyUI 云服务（Comfy Cloud、RunningHub 一类），不在本 ADR 范围：前者是市场条目类型的扩展，后者走声明式端点。
