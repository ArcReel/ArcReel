...
- **SSE 推 entry 流**（事件 id 即 seq，断线按 cursor 续传）；turn 结构组装（合并连续 assistant、tool_result 回填、task 就地更新）是前端投影层的纯函数。
- **流式预览（draft）是服务端内存态**，身份为消息 id，消息完成时被同 id 的日志条目精确替换；不入日志，崩溃即丢（与 agent 记忆一致）。
...
否决的替代方案：收敛去重逻辑而不统一消息身份（内容比对的边界渗漏无法消除）；直接以 transcript 为 UI 日志（写入时机由 SDK 控制，无法在用户消息受理时分配身份；UI 专有条目污染 resume；subagent 独立 subpath 与 UI 时序不同构）；新旧协议 feature flag 双轨（前端须并行维护两套 store 逻辑）。
