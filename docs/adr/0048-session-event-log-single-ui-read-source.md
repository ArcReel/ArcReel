...
Agent 对话的 UI 真相若分散在多个来源（SDK transcript、内存 buffer、流式预览）且各来源没有共享的消息身份，一致性就只能靠内容比对启发式缝合，消息丢失/重复与渲染不一致会随边界情形持续渗漏。决定新增**会话事件日志**（每会话单调 seq 的持久化独立表）作为 UI 时间线唯一读源：
