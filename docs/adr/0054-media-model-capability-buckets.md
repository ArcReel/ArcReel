...
- 已入队任务的执行身份在入队时钉住：解析出的 `provider/model` 以桶键形态（`video_provider_<cap>`）写进 task payload，执行与中断续跑都按它执行。task 行只存 provider_id 时 model 会按彼时配置重解析（自定义供应商更是补不出默认 model 而整段 payload 优先级落空），排队期间改配置即等于换模型——与「不静默换模型」相悖。钉住的身份不过能力闸（已入队任务按 payload 照常执行），但仍校验身份是否解析得到：供应商下线、模型被删或禁用、endpoint 改成其它媒体类型时报错，不改用其他身份执行——换身份续跑会拿另一个 backend 去轮原身份的 `provider_job_id`。
