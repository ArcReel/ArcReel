...
- 成片音轨（有无音轨、开关是否可控）同属 backend `VideoCapabilities`，且**按桶各声明一份**：`audio_track` 描述 i2v 路径，`reference_route_audio_track` 描述 r2v 路径（缺省即两条路径同形），取值一律经 `audio_track_for_route(capability)`。逐桶而非逐模型，是因为同一模型的两条路径可以给出不同答案——可灵 v3-omni 的多图主体子路径（R2V）原生 schema 不含音轨开关 `sound`，成片必然无声，而它的图生子路径开关可控。展示层（设置页开关置灰）、入队预检（恒有声模型拒绝「关闭音频」）与声音一致性派生都读同一份逐桶声明，故参考路线如实呈现为「该路径无音频开关」，而不是放行一个执行期必然被丢弃的开关。未声明的 model 取 `controllable`（无信号不收紧），不把能力不明的型号谎报成开关失效。
