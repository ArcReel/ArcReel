...
- 执行模型、分辨率、收窄后档位、端点固定与音轨事实由「视频请求事实」对每次请求求值一次，读侧与执行侧只差身份来源；参考生视频项目中 i2v 与 r2v 两个执行模型的分辨率各按自己的模型键存取，无参考图单元的桶按可用参考图判定，见 `docs/adr/0085`。
