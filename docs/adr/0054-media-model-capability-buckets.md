...
- 已入队任务的执行身份在入队时钉住：解析出的 `provider/model` 以桶键形态（`video_provider_<cap>`）写进 task payload，执行与中断续跑都按它执行。task 行只存 provider_id 时 model 会按彼时配置重解析（自定义供应商更是补不出默认 model 而整段 payload 优先级落空），排队期间改配置即等于换模型——与「不静默换模型」相悖。
