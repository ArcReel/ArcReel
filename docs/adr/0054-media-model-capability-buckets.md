...
- 前端呈现与 per-model 存储按**执行模型**取键：凡按模型查能力或按模型存配置的界面元素（能力面板、`model_settings` 下的分辨率等），一律取当前配置真正会执行的模型——细分桶生效时是桶内模型，否则是默认层穿透演算的结果；视频侧先按 generation_mode 定桶再求值，与后端定桶口径同源。此处的 generation_mode 是项目设置页持有的项目级值，不含剧集覆盖——剧集覆盖只影响上一条读侧的能力查询与执行判定；项目存在剧集覆盖时，UI 展示与 `model_settings` 存储键仍按项目级 generation_mode 求值，可能与该集实际执行的模型不同。默认层模型不作查询或存储的键，除非它就是执行模型。
