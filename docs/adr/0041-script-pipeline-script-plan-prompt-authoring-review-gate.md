...
- reference_video 的 prompt_authoring prompt（`lib/prompts/prompt_builders_reference.py`）以结构化 unit 数据为基底组装，不解析自由文本；web 端 script_plan 预览与编辑按结构渲染。
...
- 两段的名字是**脚本规划（`script_plan`）**与**提示词编写（`prompt_authoring`）**（术语见 `CONTEXT.md`）；落盘文件名、`project.json` 分集字段、剧本 metadata 字段与产物 kind 一律用这两个标识，存量项目由 v9→v10 schema 迁移改名。唯一的保留项是产物指纹的两个 token——`lib.artifacts.artifact_provenance` 的 `SCRIPT_PLAN_BASIS_KIND`（值 `structured-content/step1`）与 `SCRIPT_PLAN_BASIS_INPUT_KEY`（值 `step1_content`）：它们参与 `basis_digest` 计算，而清单只落 digest、不落输入，改值等于把全部存量脚本规划与剧本产物判成 stale。值是持久化格式，不随术语改名。
