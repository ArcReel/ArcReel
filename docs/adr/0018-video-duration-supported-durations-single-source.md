...
- fail loud 守的是「型号声明缺失」，不是「时长这一维在该端点上不存在」。ComfyUI 端点的 `frames` 未绑定时时长整维不由 ArcReel 驱动（见 `docs/adr/0082`），空集是合法状态：它带独立标志与声明缺失区分，全部消费点按该标志放行，本决定的 fail loud 立场不因此放宽。
