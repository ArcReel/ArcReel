...
- `reference_video` 的可恢复身份不能只靠 job_id。任务在首次 provider submit 紧前持久化专用、版本化且严格校验的 execution checkpoint，同一次事务写入实际 provider。checkpoint 冻结 task/project/script/unit 坐标、能力桶、provider/model/backend、自定义 endpoint 协议、最终 prompt/时长/请求设置、执行开始时已通过闸口的旁白事实，以及实际 provider 媒体的逻辑身份、字节摘要与受控定位。这些事实不写回 enqueue payload，也不与 task 状态、job_id 或 Artifact Manifest 的 current 指针混合。
- `reference_video` 孤儿按 checkpoint/job 组合穷举分流：两者均无是未到可恢复提交边界的普通中断；只有 checkpoint 表示已尝试 submit 但没有可轮询句柄，不伪装成可恢复；两者均有且 checkpoint 能严格解析/绑定才可 resume；有 job 但 checkpoint 缺失、损坏或坐标不匹配时为不可恢复的执行身份。resume 只从 checkpoint 回放提交时请求，当前 project 仅用于取凭证并构造同一 backend；provider/model/backend 或自定义 endpoint 发生漂移时显式失败。
- 实际附加到 reference provider 请求的图像与音色参考在 submit 前物化到 task-local 不可变 staging，provider 只读该字节；不参与 provider 请求的 TTS 旁白音频不复制。staging 采用项目边界校验、字节摘要复核、原子发布与幂等冲突检查；失败、取消或任务终态只清这个任务的 provider-media 目录，不删已登记的付费版本历史。
