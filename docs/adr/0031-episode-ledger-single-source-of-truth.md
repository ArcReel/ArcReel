...
- 没有 source_range 的账本条目（存量项目遗留、或手动预拆分上传）不参与规划：物理集文件即其最终记录，下游消费不受影响，plan 与部分重置则拒绝执行并指路全量重置。不按内容反推坐标——派生文件与源文的差异只可能来自人工编辑，近似锚定会让账本失真，宁可让老项目一次性重置进入新机制。
