...
- 回填比对允许无损预处理（统一换行符、Unicode NFC），但不做模糊匹配：派生文件与源文的语义性差异只可能来自人工编辑，近似锚定会让账本失真。unanchored 是诚实降级而非失败——物理文件仍是该集最终记录，下游消费不受影响，仅不参与重新规划。
