...
- 目录替换是两次改名（原目录 → `.<项目名>.v6-rollback-<hex>`，staging 树 → 项目目录），窗口内进程被硬杀时项目目录不存在、rollback 目录残留——恢复责任归启动期扫描：runner 在遍历项目前认领 rollback 目录，对应项目目录缺失时原子改回并记录，项目在本轮继续迁移。
- 项目目录在位时 rollback 目录只是备份，与遗留 staging 树一并按版本化备份的年龄闸口清理；项目目录缺失期间不清理，它是唯一的项目数据。
- staging 树是整个项目目录的副本，替换前按项目体量做磁盘容量预检，空间不足以可读错误失败，原目录不被触碰。
