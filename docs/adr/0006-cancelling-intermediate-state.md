...
- cancel 信号通道用 worker 内存 `dict[task_id, asyncio.Task]`：ArcReel 的 GenerationWorker 始终与 server 主进程捆绑在同一个 uvicorn 进程（参见 `lib/generation_worker.py` 模块文档字符串），无跨进程传 cancel 信号的需要。后续若架构变成多 worker（目前没有规划），需要在 DB 加 `cancel_requested` 标记 + worker 轮询，但本 ADR 不预留。
