...
- Repository 层状态转移 invariant：**所有终态/中间态写入都必须由 SQL `WHERE current_status=...` 守卫，防止越权转移**。具体：`mark_task_succeeded` / `mark_task_failed` 用 `WHERE status='running'`；`mark_task_cancelled` 同时承担两个入口（cancel queued + worker finally 兜底 cancelling），用 `WHERE status IN ('queued', 'cancelling')`；`mark_task_cancelling` 用 `WHERE status='running'`。这条 invariant 是「0-rows 即转 cancelled」协议成立的基础——没有 WHERE 守卫，并发写入可能让 cancelling 又被一个迟到的 succeeded 覆盖。
