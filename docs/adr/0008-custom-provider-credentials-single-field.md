...
- **未来 plugin 机制必须延续这条约束**：当运行时 plugin 机制落地时，plugin endpoint 同样只能接 `api_key` + `base_url`；plugin 想暴露多字段协议须自带"内置 provider 风格"声明（且该路径等同于 plugin 自己提供 PROVIDER_REGISTRY 扩展，复杂度显著上升，本 ADR 范围不预设）。这条约束让 plugin 接口面保持稳定。
- **CONTEXT.md 已经把这条原则收入 glossary**（「自定义供应商（custom provider）」条目第二句），本 ADR 给出的是"为什么不扩展凭证模型"的决策理由，与 glossary 描述"是什么"形成互补。任何后续 PR 想破例（加 JSON 凭证字段、加 secondary key 字段）须先 deprecate 本 ADR。
