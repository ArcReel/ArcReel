...
> `docs/adr/0058` 已替代本 ADR 关于“同一场景可交错两类发声”的合法性、utterance 判别、发声归属与字幕时序决定：一个视频生成单元只能有一种发声归属；人物对白、人物内心独白和人物画外解说都归属于人物并使用视频原生声；只有不归属于人物的叙述旁白使用 TTS；字幕按实际媒体外边界机械顺排。本 ADR 关于 drama 在单一发声归属内以有序 utterances 统一口播真相、保留 `source_text` 与单向时长约束的决定继续有效。
